...
import static com.linzen.consts.AuthConsts.PARAMS_LINZEN_TICKET;
 ...
 * @date 2023-04-01
 */
@Tag(name = "用户登录系统", description = "oauth")
@Slf4j
@RestController
...
public class LoginController {
    ...
    public ServiceResult<LoginVO> login(@RequestParam Map<String, String> parameters) throws LoginException {
    ...
    public ServiceResult<String> lockScreen(@RequestBody LoginForm loginForm) throws LoginException {
        SysUserEntity userEntity = userService.getUserByAccount(loginForm.getAccount());
        if (userEntity == null) {
    ...
    public ServiceResult logout(@PathVariable(value = "grandtype", required = false) String grandtype) {
        long millis = System.currentTimeMillis();
        TokenGranter tokenGranter = tokenGranterBuilder.getGranterByLogin(grandtype);
        if (tokenGranter != null) {
            UserInfo userInfo = UserProvider.getUser();
            logService.writeLogAsync(userInfo.getUserId(), userInfo.getUserName() + "/" + userInfo.getUserAccount(), "退出登录", userInfo, 1, 1, (System.currentTimeMillis() - millis));
    ...
    public ServiceResult<PcUserVO> currentUser(String type, String systemCode) throws LoginException {
        ...
        UserInfo userInfo = UserProvider.getUser();
        if (DeviceType.TEMPORALITIES.getDevice().equals(userInfo.getLoginDevice())) {
        ...
        return ServiceResult.success(pcUserVO);
    }
    ...
    public ServiceResult<String> updatePasswordMessage() {
    ...
    public ServiceResult<String> logoutCurrentUser() {
        UserInfo userInfo = UserProvider.getUser();
        if (userInfo.getIsAdministrator() != null && UserProvider.getUser().getIsAdministrator()) {
            return ServiceResult.error("管理员不能注销");
        }
    ...
    public ServiceResult<LoginModel> check(@PathVariable("account") String account) throws LoginException {
        ...
        if (Objects.nonNull(sysConfigInfo) && "1".equals(String.valueOf(sysConfigInfo.getEnableVerificationCode()))) {
            ...
            return ServiceResult.success(loginModel);
        }
    ...
    public ServiceResult<LoginConfigModel> getLoginConfig() {
        ...
        if (oauthConfig.getSsoEnabled()) {
            ...
            loginConfigModel.setTicketParams(PARAMS_LINZEN_TICKET);
        } else {
            //追加第三方登录配置
            List<SocialsUserVo> loginList = socialsUserController.getLoginList(PARAMS_LINZEN_TICKET.toUpperCase());
        ...
        return ServiceResult.success(loginConfigModel);
    }
    ...
    public ServiceResult<String> getTicket() {
        ...
        return ServiceResult.success(ticketModel.getTicketTimeout().toString(), ticket);
    }
    ...
    public ServiceResult<LoginTicketModel> getTicketStatus(@PathVariable("ticket") String ticket) {
        ...
        return ServiceResult.success(ticketModel);
    }
    ...
    public ServiceResult<String> resetOfficialPassword(@PathVariable("mobile") String mobile, @PathVariable("code") String code) throws LoginException {
        //校验验证码
        ...
        userEntity.setPassword("e10adc3949ba59abbe56e057f20f883e");// 重置密码 123456
        ...
        if (result) {
            return ServiceResult.success(MsgCode.LOG205.get());
        } else {
            return ServiceResult.error(MsgCode.LOG206.get());
        }
    }
