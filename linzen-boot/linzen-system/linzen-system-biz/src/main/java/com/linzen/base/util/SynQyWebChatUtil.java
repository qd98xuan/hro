...
import com.linzen.message.model.message.OraganizeListVO;
...
public class SynQyWebChatUtil {
    ...
    public static JSONObject getLinkedcorpUserById(String id, String accessToken) throws WxErrorException {
        ...
        JSONObject rstObj = HttpUtil.httpRequest(String.format(GET_LINKEDCORP_USER, accessToken), "POST", JsonUtil.createObjectToString(userObject));
        if (HttpUtil.isWxError(rstObj)) {
            codeFlag = false;
            errorMsg = rstObj.toString();
        }else{
    ...
    public static JSONObject getUserList(String id, String isGetChild, String accessToken) throws WxErrorException {
        ...
        if (HttpUtil.isWxError(rstObj)) {
            ...
            userList = JsonUtil.createJsonToList(rstObj.getJSONArray("userlist"));
        }
        retMsg.put("code",codeFlag);
        retMsg.put("error",errorMsg);
     ...
     * @param listByOrder
     */
    public static void getOrganizeTreeToList(OraganizeListVO selectorVO, Map<String, SysOrganizeEntity> organizeList, List<SysOrganizeEntity> listByOrder){
        if(selectorVO.isHasChildren()) {
            ...
            while (iterator.hasNext()) {
                ...
                SysOrganizeEntity entity = organizeList.get(organizeSelectorVO.getId());
