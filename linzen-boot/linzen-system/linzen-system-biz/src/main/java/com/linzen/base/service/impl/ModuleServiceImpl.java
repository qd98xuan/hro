...
import com.linzen.base.mapper.ModuleMapper;
import com.linzen.base.model.module.ModuleExportModel;
...
public class ModuleServiceImpl extends SuperServiceImpl<ModuleMapper, ModuleEntity> implements ModuleService {
    ...
    public List<ModuleEntity> getList(boolean filterFlowWork, List<String> moduleAuthorize, List<String> moduleUrlAddressAuthorize) {
        ...
        // 移除工作流程菜单
        if (filterFlowWork) {
            List<String> moduleCode = LinzenConst.MODULE_CODE;
    ...
    @Override
    public List<ModuleEntity> getList(String systemId, String category, String keyword, Integer type, Integer delFlag, String parentId, boolean release) {
        // 定义变量判断是否需要使用修改时间倒序
        boolean flag = false;
        QueryWrapper<ModuleEntity> queryWrapper = new QueryWrapper<>();
        QueryWrapper<SysSystemEntity> query = new QueryWrapper<>();
        query.lambda().eq(SysSystemEntity::getEnCode, LinzenConst.MAIN_SYSTEM_CODE);
        query.lambda().select(SysSystemEntity::getId);
        SysSystemEntity systemEntity = systemService.getOne(query);
        ...
        if (delFlag != null) {
            flag = true;
            queryWrapper.lambda().eq(ModuleEntity::getEnabledMark, delFlag);
        }
        ...
        queryWrapper.lambda().orderByAsc(ModuleEntity::getSortCode)
                .orderByDesc(ModuleEntity::getCreatorTime);
        if (flag) {
            queryWrapper.lambda().orderByDesc(ModuleEntity::getUpdateTime);
        }
        // 移除工作流程菜单
        List<String> moduleCode = LinzenConst.MODULE_CODE;
        queryWrapper.lambda().notIn(ModuleEntity::getEnCode, moduleCode);
...
//                        indexGridField6Models.addAll(JsonUtil.createJsonToList(JsonUtil.createJsonToListMap(column.get("columnList").toString()),IndexGridField6Model.class));
...
//            }
    ...
    @Override
    @DSTransactional
    public ServiceResult importData(ModuleExportModel exportModel, Integer type) throws DataBaseException {
        try {
            ...
            //按钮
            List<ModuleButtonEntity> buttonEntityList = JsonUtil.createJsonToList(exportModel.getButtonEntityList(), ModuleButtonEntity.class);
            //新ID映射
            ...
            //列表
            List<ModuleColumnEntity> columnEntityList = JsonUtil.createJsonToList(exportModel.getColumnEntityList(), ModuleColumnEntity.class);
            ...
            //表单
            List<ModuleFormEntity> formEntityList = JsonUtil.createJsonToList(exportModel.getFormEntityList(), ModuleFormEntity.class);
            ...
            List<ModuleDataAuthorizeEntity> authorizeEntityList = JsonUtil.createJsonToList(exportModel.getAuthorizeEntityList(), ModuleDataAuthorizeEntity.class);
            ...
            //数据权限方案
            List<ModuleDataAuthorizeSchemeEntity> schemeEntityList = JsonUtil.createJsonToList(exportModel.getSchemeEntityList(), ModuleDataAuthorizeSchemeEntity.class);
    ...
    public List<ModuleEntity> getModuleBySystemIds(List<String> ids, List<String> moduleAuthorize, List<String> moduleUrlAddressAuthorize) {
        ...
        queryWrapper.lambda().notIn(ModuleEntity::getEnCode, LinzenConst.MODULE_CODE);
