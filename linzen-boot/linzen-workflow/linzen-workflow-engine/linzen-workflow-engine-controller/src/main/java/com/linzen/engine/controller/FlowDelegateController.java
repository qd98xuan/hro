...
import com.linzen.engine.model.flowdelegate.*;
...
public class FlowDelegateController extends SuperController<FlowDelegateService, FlowDelegateEntity> {
    ...
    @GetMapping
    public ServiceResult<PageListVO<FlowDelegatListVO>> list(FlowDelegatePagination pagination) {
        ...
        List<FlowDelegatListVO> listVO = JsonUtil.createJsonToList(list, FlowDelegatListVO.class);
    ...
    public ServiceResult<FlowDelegateInfoVO> info(@PathVariable("id") String id) throws DataBaseException {
    ...
    public ServiceResult create(@RequestBody @Valid FlowDelegateCrForm flowDelegateCrForm) {
        FlowDelegateEntity entity = BeanUtil.toBean(flowDelegateCrForm, FlowDelegateEntity.class);
        if (entity.getUserId().equals(entity.getToUserId())) {
            return ServiceResult.error("委托人和被委托人相同，委托失败！");
        }
        if (alreadyDelegate(flowDelegateCrForm, null)) {
            return ServiceResult.error("操作失败，同一时间内有相同流程的委托！");
        }
        ...
        if (alreadyDelegate(flowReverse, null)) {
            return ServiceResult.error("操作失败，同一时间内有相同流程，不能相互委托！");
        }
    ...
    public ServiceResult update(@PathVariable("id") String id, @RequestBody @Valid FlowDelegateUpForm flowDelegateUpForm) {
        FlowDelegateEntity entity = BeanUtil.toBean(flowDelegateUpForm, FlowDelegateEntity.class);
        if (entity.getUserId().equals(entity.getToUserId())) {
            return ServiceResult.error("委托人和被委托人相同，委托失败");
        }
    ...
    public ServiceResult stop(@PathVariable("id") String id) {
        FlowDelegateEntity entity = flowDelegateService.getInfo(id);
        if (entity != null) {
            Date date = new Date();
            ...
            return ServiceResult.success(MsgCode.SU008.get());
        }
    ...
    public ServiceResult getuser(@RequestParam("flowId") String flowId, @RequestParam("userId") String userId, @RequestParam("touserId") String touserId) {
    ...
    @GetMapping("/getflow")
    public ServiceResult<PageListVO<FlowPageListVO>> getflow(FlowPagination pagination) {
        List<FlowPageListVO> getflow = flowDelegateService.getflow(pagination);
        PaginationVO paginationVO = BeanUtil.toBean(pagination, PaginationVO.class);
        List<FlowPageListVO> listVO = JsonUtil.createJsonToList(getflow, FlowPageListVO.class);
        return ServiceResult.pageList(listVO, paginationVO);
    }
    ...
    public ServiceResult<ListVO<FlowCandidateUserModel>> getUserListByFlowId(@RequestParam("flowId") String flowId) throws WorkFlowException {
        ListVO<FlowCandidateUserModel> userListByFlowId = flowDelegateService.getUserListByFlowId(flowId);
        return ServiceResult.success(userListByFlowId);
    }
}
