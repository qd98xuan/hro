...
import com.linzen.mapper.EmployeeMapper;
...
//        Map<String, Object> queryParam = OptimizeUtil.queryParam(pagination);
    ...
    public EmployeeImportVO importData(List<EmployeeModel> dt) {
        ...
        List<EmployeeEntity> entitys = JsonUtil.createJsonToList(dt, EmployeeEntity.class);
        //记录成功了几条
        int sum=0;
        //记录第几条失败
        int num=0;
        ...
        if(vo.getFnum()>0){
            vo.setResultType(1);
            vo.setFailResult(JsonUtil.createJsonToList(errList, EmployeeModel.class));
            return vo;
        }else{
            vo.setResultType(0);
            return vo;
