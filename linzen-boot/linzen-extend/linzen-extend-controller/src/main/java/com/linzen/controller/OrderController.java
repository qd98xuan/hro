...
public class OrderController extends SuperController<OrderService, OrderEntity> {
    ...
    public ServiceResult<PageListVO<OrderListVO>> list(PaginationOrder paginationOrder) {
        List<OrderEntity> data = orderService.getList(paginationOrder);
        List<OrderListVO> listVO = JsonUtil.createJsonToList(data, OrderListVO.class);
        ...
        for (OrderListVO order : listVO) {
            SysUserEntity user = userList.stream().filter(t -> t.getId().equals(order.getCreatorUserId())).findFirst().orElse(null);
        ...
        PaginationVO paginationVO = BeanUtil.toBean(paginationOrder, PaginationVO.class);
    ...
    @SaCheckPermission("extend.order")
    public ServiceResult<ListVO<OrderEntryListVO>> orderEntryList(@PathVariable("id") String id) {
        List<OrderEntryEntity> data = orderService.getOrderEntryList(id);
        List<OrderEntryListVO> result = JsonUtil.createJsonToList(data, OrderEntryListVO.class);
    ...
    @SaCheckPermission("extend.order")
    public ServiceResult<ListVO<OrderReceivableListVO>> orderReceivableList(@PathVariable("id") String id) {
        List<OrderReceivableEntity> data = orderService.getOrderReceivableList(id);
        List<OrderReceivableListVO> result = JsonUtil.createJsonToList(data, OrderReceivableListVO.class);
        ListVO vo = new ListVO();
        vo.setList(result);
        return ServiceResult.success(vo);
    }
    ...
    @SaCheckPermission("extend.order")
    public ServiceResult<OrderInfoVO> info(@PathVariable("id") String id) {
        ...
        OrderInfoVO infoVo = BeanUtil.toBean(orderEntity, OrderInfoVO.class);
        if (infoVo != null) {
            ...
            if (StringUtil.isNotEmpty(infoVo.getCreatorUserId())) {
                SysUserEntity creator = userService.getInfo(infoVo.getCreatorUserId());
                infoVo.setCreatorUserId(creator != null ? creator.getRealName() + "/" + creator.getAccount() : "");
            }
            if (StringUtil.isNotEmpty(infoVo.getUpdateUserId())) {
                SysUserEntity userEntity = userService.getInfo(infoVo.getUpdateUserId());
                infoVo.setUpdateUserId(userEntity != null ? userEntity.getRealName() + "/" + userEntity.getAccount() : "");
            }
        }
        return ServiceResult.success(infoVo);
    }
    ...
    @SaCheckPermission("extend.order")
    public ServiceResult<OrderInfoVO> prevInfo(@PathVariable("id") String id) throws DataBaseException {
    ...
    @SaCheckPermission("extend.order")
    public ServiceResult<OrderInfoVO> nextInfo(@PathVariable("id") String id) throws DataBaseException {
    ...
    @SaCheckPermission("extend.order")
    public ServiceResult<DownloadVO> exportExcel(@PathVariable("id") String id) {
        OrderEntity orderEntity = orderService.getInfo(id);
        if (orderEntity != null) {
            if (StringUtil.isNotEmpty(orderEntity.getCreatorUserId())) {
                SysUserEntity user = userService.getInfo(orderEntity.getCreatorUserId());
                orderEntity.setCreatorUserId(user != null ? user.getRealName() + "/" + user.getAccount() : "");
            }
            if (StringUtil.isNotEmpty(orderEntity.getUpdateUserId())) {
                SysUserEntity user = userService.getInfo(orderEntity.getUpdateUserId());
                orderEntity.setUpdateUserId(user != null ? user.getRealName() + "/" + user.getAccount() : "");
            }
            OrderExportModel exportModel = BeanUtil.toBean(orderEntity, OrderExportModel.class);
            //保存数据
        ...
        return ServiceResult.error("未能找到此订单");
    }
    ...
    @SaCheckPermission("extend.order")
    public ServiceResult create(@PathVariable("id") String id, @RequestBody @Valid OrderForm orderForm) throws WorkFlowException {
        OrderEntity orderEntity = BeanUtil.toBean(orderForm, OrderEntity.class);
        orderEntity.setId(id);
        List<OrderEntryEntity> orderEntryList = JsonUtil.createJsonToList(orderForm.getGoodsList(), OrderEntryEntity.class);
        List<OrderReceivableEntity> orderReceivableList = JsonUtil.createJsonToList(orderForm.getCollectionPlanList(), OrderReceivableEntity.class);
    ...
    @SaCheckPermission("extend.order")
    public ServiceResult update(@PathVariable("id") String id, @RequestBody @Valid OrderForm orderForm) throws WorkFlowException {
        OrderEntity orderEntity = BeanUtil.toBean(orderForm, OrderEntity.class);
        List<OrderEntryEntity> orderEntryList = JsonUtil.createJsonToList(orderForm.getGoodsList(), OrderEntryEntity.class);
        List<OrderReceivableEntity> orderReceivableList = JsonUtil.createJsonToList(orderForm.getCollectionPlanList(), OrderReceivableEntity.class);
    ...
    public ServiceResult<ListVO<OrderGoodsVO>> customerList(Page page) {
        ...
        List<OrderCustomerVO> list = JsonUtil.createJsonToList(data, OrderCustomerVO.class);
        ListVO vo = new ListVO();
        vo.setList(list);
        return ServiceResult.success(vo);
    }
