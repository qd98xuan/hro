package com.linzen.permission.util;
...
public class PermissionUtil {
    ...
    private static LinkedList<String> getOrganizeInfos(LinkedList<String> organizeInfo, String organizeId, Integer infoType, OrganizeService organizeService){
        SysOrganizeEntity infoEntity = organizeService.getInfo(organizeId);
        if(infoEntity != null){
    ...
    public static List<String> getOrgIdsByFormTree(OrganizeService organizeService, List<List<String>> organizeIdsTree) throws DataBaseException {
        ...
        if(count != orgIds.size()){
            throw new DataBaseException(MsgCode.FA026.get());
        }
        return orgIds;
    }
