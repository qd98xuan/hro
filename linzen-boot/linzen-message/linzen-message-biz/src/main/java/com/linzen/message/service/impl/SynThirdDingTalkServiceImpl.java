...
import com.linzen.base.util.RegexUtils;
...
import com.linzen.message.model.message.DingTalkUserModel;
...
//                        dingDeptList = JsonUtil.createJsonToList(deptObject.getObject("department", List.class), DingTalkDeptModel.class);
//                    } else {
...
//    }
     ...
     */
    @Override
    public JSONObject createDepartmentSysToDing(boolean isBatch, SysOrganizeEntity deptEntity, String accessToken) {
        BaseSystemInfo config = getDingTalkConfig();
        String corpId = config.getDingSynAppKey();
        String corpSecret = config.getDingSynAppSecret();
        String compValue = SynThirdConsts.OBJECT_TYPE_COMPANY;
        ...
        if(isBatch || dingIsSyn==1){
            ...
            if(isBatch) {
                ...
                List<SynThirdInfoEntity> synThirdInfoList = synThirdInfoService.syncThirdInfoByType(SynThirdConsts.THIRD_TYPE_DING,SynThirdConsts.DATA_TYPE_ORG, SynThirdConsts.THIRD_TYPE_DING_To_Sys);
                tokenObject = SynDingTalkUtil.getAccessToken(corpId, corpSecret);
                access_token = tokenObject.getString("access_token");
            }
     ...
     */
    @Override
    public JSONObject updateDepartmentSysToDing(boolean isBatch, SysOrganizeEntity deptEntity, String accessToken) {
        BaseSystemInfo config = getDingTalkConfig();
        String corpId = config.getDingSynAppKey();
        String corpSecret = config.getDingSynAppSecret();
        String compValue = SynThirdConsts.OBJECT_TYPE_COMPANY;
    ...
    public JSONObject createDepartmentDingToSys(boolean isBatch, DingTalkDeptModel deptEntity,String accessToken) {
        ...
        if(isBatch || dingIsSyn==1){
            ...
            if(isDeptDiff || tag) {
                ...
                if(isBatch || dingIsSyn==1) {
                    // 新增保存组织
                    SysOrganizeEntity newOrg = new SysOrganizeEntity();
...
//        String thirdObjId = "";
        ...
        String dingDeptName = deptEntity.getName();
        Long dingParentId = deptEntity.getParentId();
        SysOrganizeEntity orgInfo = new SysOrganizeEntity();
...
//                            orgInfo.setCategory(SynThirdConsts.OBJECT_TYPE_DEPARTMENT);
    ...
    public JSONObject deleteDepartmentDingToSys(boolean isBatch, String thirdObjId) {
        ...
        // 获取当前第三方同步表对应记录
        SynThirdInfoEntity synThirdInfoEntity = synThirdInfoService.getInfoByThirdObjId(SynThirdConsts.THIRD_TYPE_DING_To_Sys,SynThirdConsts.DATA_TYPE_ORG,thirdObjId);
        ...
        if(synThirdInfoEntity!=null) {
            if(isBatch || dingIsSyn==1){
                ...
                if (!"".equals(String.valueOf(sysObjId)) && !"null".equals(String.valueOf(sysObjId))){
                    SysOrganizeEntity sysOrgEntity = organizeService.getInfo(sysObjId);
                ...
                saveSynThirdInfoEntity(SynThirdConsts.OBJECT_OP_UPD, synThirdInfoEntity, Integer.parseInt(SynThirdConsts.THIRD_TYPE_DING_To_Sys),
                        Integer.parseInt(SynThirdConsts.DATA_TYPE_ORG), synThirdInfoEntity.getSysObjId(), thirdObjId, SynThirdConsts.SYN_STATE_NO, deptFlag + "系统未设置单条同步");
...
//    }
     ...
     * @return
     */
    public JSONObject setDingUserObject(SysUserEntity userEntity) throws ParseException {
        DingTalkUserModel userModel = new DingTalkUserModel();
        JSONObject retMsg = new JSONObject();
        retMsg.put("code", true);
        retMsg.put("error", "");
        ...
        SysPositionEntity positionEntity = positionService.getInfo(userEntity.getPositionId());
        String jobName = "";
        if(positionEntity!=null){
            jobName = positionEntity.getFullName();
            userModel.setTitle(jobName);
        ...
        SynThirdInfoEntity synThirdInfoEntity = synThirdInfoService.getInfoBySysObjId(SynThirdConsts.THIRD_TYPE_DING,SynThirdConsts.DATA_TYPE_ORG,userEntity.getOrganizeId());
        // retMsg = checkDepartmentSysToDing(synThirdInfoEntity,dingDeptList);
        List<SysUserRelationEntity> userRelationList = userRelationService.getListByObjectType(userEntity.getId(),"Organize");
...
//                        dingUserList = JsonUtil.createJsonToList(userObject.getObject("userlist", List.class), DingTalkUserModel.class);
//                    } else {
//                        synState = SynThirdConsts.SYN_STATE_FAIL;
...
//    }
     ...
     */
    @Override
    public JSONObject createUserSysToDing(boolean isBatch, SysUserEntity userEntity, String accessToken) throws ParseException {
        BaseSystemInfo config = getDingTalkConfig();
        String corpId = config.getDingSynAppKey();
        String corpSecret = config.getDingSynAppSecret();
        // 单条记录执行时,受开关限制
        ...
        if (isBatch || dingIsSyn==1){
            if(isBatch){
                ...
                synThirdInfoService.syncThirdInfoByType(SynThirdConsts.THIRD_TYPE_DING,SynThirdConsts.DATA_TYPE_ORG, SynThirdConsts.THIRD_TYPE_DING_To_Sys);
                synThirdInfoService.syncThirdInfoByType(SynThirdConsts.THIRD_TYPE_DING,SynThirdConsts.DATA_TYPE_USER, SynThirdConsts.THIRD_TYPE_DING_To_Sys);
                tokenObject = SynDingTalkUtil.getAccessToken(corpId, corpSecret);
                access_token = tokenObject.getString("access_token");
            }
     ...
     */
    @Override
    public JSONObject updateUserSysToDing(boolean isBatch, SysUserEntity userEntity, String accessToken) throws ParseException {
        BaseSystemInfo config = getDingTalkConfig();
        String corpId = config.getDingSynAppKey();
        String corpSecret = config.getDingSynAppSecret();
        // 单条记录执行时,受开关限制
        ...
        if(synThirdInfoEntity!=null) {
            if(isBatch || dingIsSyn==1) {
                ...
                if(isBatch){
                    ...
                    List<SynThirdInfoEntity> synThirdInfoList = synThirdInfoService.syncThirdInfoByType(SynThirdConsts.THIRD_TYPE_DING,SynThirdConsts.DATA_TYPE_USER, SynThirdConsts.THIRD_TYPE_DING_To_Sys);
                    tokenObject = SynDingTalkUtil.getAccessToken(corpId, corpSecret);
                    access_token = tokenObject.getString("access_token");
                }
    ...
    public JSONObject createUserDingToSys(boolean isBatch, OapiV2UserListResponse.ListUserResponse dingUserModel, String accessToken) throws Exception {
        ...
        if(isBatch || dingIsSyn==1){
            // 检测账户唯一
            SysUserEntity userAccount = userService.getUserByMobile(dingMobile);
            if (userAccount!=null) {
                // 查询用户id在不在同步表
...
//                    userRelationService.syncDingUserRelation(sysObjId,deptIdList);
     ...
     */
    @Override
    public JSONObject updateUserDingToSys(boolean isBatch, SysUserEntity userEntity, String accessToken) throws ParseException {
        BaseSystemInfo config = getDingTalkConfig();
        String corpId = config.getDingSynAppKey();
        String corpSecret = config.getDingSynAppSecret();
        // 单条记录执行时,受开关限制
...
//            }
    ...
    public JSONObject updateUserDingToSystem(boolean isBatch, OapiV2UserListResponse.ListUserResponse dingUserModel) throws Exception {
        ...
        if (isBatch || dingIsSyn==1){
            ...
            if(synThirdInfoEntity!=null && StringUtil.isNoneBlank(synThirdInfoEntity.getSysObjId())){
                ...
                SysUserEntity info = userService.getInfo(sysObjId);
                if(info==null){
                    synState = SynThirdConsts.SYN_STATE_NO;
    ...
    @Override
    public void deleteSyncByBothWay(String thirdTypeDing, String id) {
        HashMap<String,String> typeMap = new HashMap<>();
        typeMap.put(SynThirdConsts.THIRD_TYPE_DING,SynThirdConsts.THIRD_TYPE_DING_To_Sys);
        typeMap.put(SynThirdConsts.THIRD_TYPE_DING_To_Sys,SynThirdConsts.THIRD_TYPE_DING);
        SynThirdInfoEntity synThirdInfoEntity = synThirdInfoService.getInfoBySysObjId(typeMap.get(thirdTypeDing),SynThirdConsts.DATA_TYPE_USER,id);
        if(synThirdInfoEntity!=null){
            synThirdInfoService.delete(synThirdInfoEntity);
        }
    }
...
//        String access_token = "";
        ...
        if(synThirdInfoEntity!=null) {
            ...
            if(isBatch || dingIsSyn==1) {
                ...
                if (!"".equals(String.valueOf(sysObjId)) && !"null".equals(String.valueOf(sysObjId))) {
                    // 获取用户信息
                    SysUserEntity userEntity = userService.getInfo(sysObjId);
                    if(userEntity!=null){
                        // 删除用户,更新为标记为不可登录
                        // 禁用登录
