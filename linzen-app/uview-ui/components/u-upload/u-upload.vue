...
<script>
	 ...
	 */
	import WCompress from '@/uni_modules/w-compress/w-compress.vue'
	export default {
		components: {
			WCompress
		},
		...
		methods: {
			...
			async uploadFile(index = 0) {
				...
				const url = this.lists[index].url
				this.compressImage(index, url)
			},
			compressImage(index, url) {
				// #ifdef H5
				this.uploadFile_(index, url)
				// #endif
				// #ifndef H5
				uni.getFileInfo({
					filePath: url,
