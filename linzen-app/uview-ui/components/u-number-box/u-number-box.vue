<template>
	<view class="u-numberbox">
		<view class="u-icon-minus" @touchstart.stop="btnTouchStart('minus')" @touchend.stop="clearTimer"
		...
		<view class="u-icon-plus" @touchstart.stop="btnTouchStart('plus')" @touchend.stop="clearTimer"
...
<script>
	...
	export default {
		...
		created() {
			this.inputVal = this.value ? Number(this.value) : this.value == 0 ? 0 : null
		},
		computed: {
			getCursorSpacing() {
				// 先将值转为px单位，再转为数值
