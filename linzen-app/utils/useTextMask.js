...
export function useTextMask(options) {
	...
	function maskCustom(str) {
		...
		if ((config.prefixType === 3 || config.prefixType === 4) && config.prefixSpecifyChar) {
			...
			if (specifyCharIndex > -1) prefixLength = config.prefixType === 3 ? specifyCharIndex : specifyCharIndex +
				config.prefixSpecifyChar.length;
		}
		...
		if ((config.suffixType === 3 || config.suffixType === 4) && config.suffixSpecifyChar) {
			...
			if (specifyCharIndex > -1)
				suffixLength = config.suffixType === 3 ? str.length - specifyCharIndex - config.suffixSpecifyChar
				.length : str.length - specifyCharIndex;
		}
