		...
		<u-form :model="dataForm" :rules="rules" ref="dataForm" :errorType="['toast']" label-position="left"
			...
			<u-form-item label="开关" prop="switch">
				<view class="u-flex u-form-item-switch">
					<linzen-switch v-model="dataForm.switch"></linzen-switch>
				</view>
			</u-form-item>
			<!-- 单选框组 -->
			...
			<u-form-item label="多选框组" prop="checkbox">
				<linzen-checkbox v-model="dataForm.checkbox" :options="checkboxList"></linzen-checkbox>
			</u-form-item>
			<!-- 下拉框组 -->
			<u-form-item label="下拉框组" prop="select">
				<linzen-select v-model="dataForm.select" placeholder="请选择下拉框组" :options="checkboxList"></linzen-select>
			</u-form-item>
			<!-- 下拉多选 -->
			<u-form-item label="下拉多选" prop="select1">
				<linzen-select v-model="dataForm.select1" placeholder="请选择下拉框组" :options="checkboxList" multiple>
				</linzen-select>
			</u-form-item>
			<!-- 级联选择 -->
			<u-form-item label="级联选择" prop="cascader">
				<linzen-cascader v-model="dataForm.cascader" placeholder="请选择级联选择" :options="cascaderList">
				</linzen-cascader>
			</u-form-item>
			<!-- 省市区 -->
			<u-form-item label="省市区" prop="areaSelect">
				<linzen-city-select v-model="dataForm.address" placeholder="请选择省市区" :level="2">
				</linzen-city-select>
			</u-form-item>
			<!-- 树形选择 -->
			<u-form-item label="树形选择" prop="treeSelect">
				<linzen-tree-select v-model="dataForm.treeSelect" placeholder="请选择树形选择" :options="treeData">
				</linzen-tree-select>
			</u-form-item>
			<!-- 文本 -->
			<linzen-text :content="text"></linzen-text>
			<!-- 上传图片 -->
			<u-form-item label="上传图片" prop="uploadImg">
				<linzen-upload v-model="dataForm.uploadImg"></linzen-upload>
			</u-form-item>
			<!-- 富文本 -->
			<!-- <u-form-item label-width="0" prop="editor">
				<linzen-editor v-model="dataForm.editor" disabled></linzen-editor>
			</u-form-item> -->
			<!-- 评分 -->
			...
			<view class="linzen-card">
				...
				<u-form-item label="时间选择" prop="time">
					<linzen-date-time type="time" v-model="dataForm.time"></linzen-date-time>
				</u-form-item>
				<!-- 日期选择 -->
				<u-form-item label="日期选择" prop="date">
					<linzen-date-time type="date" v-model="dataForm.date"></linzen-date-time>
				</u-form-item>
			</view>
			<view class="linzen-card">
				<!-- 时间选择 -->
				...
				<!-- 日期选择 -->
				<u-form-item label="日期选择" prop="date">
					<linzen-date-time type="datetime" v-model="dataForm.date"></linzen-date-time>
				</u-form-item>
			</view>
			<!-- 公司组件 -->
			<u-form-item label="公司组件" prop="organizeSelect">
				<linzen-com-select v-model="dataForm.organizeSelect" placeholder="请选择">
				</linzen-com-select>
			</u-form-item>
			<!-- 部门组件 -->
			<u-form-item label="部门组件" prop="depSelect">
				<linzen-postordep-select type="department" v-model="dataForm.depSelect"></linzen-postordep-select>
			</u-form-item>
			<!-- 岗位组件 -->
			<u-form-item label="岗位组件" prop="posSelect">
				<linzen-postordep-select type="position" v-model="dataForm.posSelect"></linzen-postordep-select>
			</u-form-item>
			<!-- 用户组件 -->
			<u-form-item label="用户组件" prop="userSelect">
				<linzen-user-select v-model="dataForm.userSelect"></linzen-user-select>
			</u-form-item>
			<!-- 分组标题 -->
			<linzen-group content="分组标题" contentPosition="right"></linzen-group>
			<!-- 系统控件(7个) + 单据组件 -->
			...
			<view class="linzen-table">
				<view class="linzen-table-item" v-for="(item,i) in dataForm.table" :key="i">
					<view class="linzen-table-item-title u-flex u-row-between">
						<text class="linzen-table-item-title-num">设计子表({{i+1}})</text>
						<view class="linzen-table-item-title-action" v-if="dataForm.table.length>1" @click="delItem(i)">删除
						</view>
					</view>
				...
				<view class="linzen-table-addBtn" @click="addItem">
					<u-icon name="plus" color="#2979ff"></u-icon>添加
				</view>
			</view>
		</u-form>
		<view class="buttom-actions">
			<u-button class="buttom-btn" @click="linzen.goBack">取消</u-button>
