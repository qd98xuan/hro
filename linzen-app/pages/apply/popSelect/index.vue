		...
		methods: {
			upCallback(page) {
				...
				method(this.modelId, query, {
					...
					var propVal = this.type === 'popup' ? 'F_Id' : 'id'
					var index = this.list.findIndex((item) => {
						return item[propVal] == this.selectId
					})
				...
				if (type === 'confirm') {
					...
					this.list.map((o, i) => {
						if (this.selectId == o[this.publicField]) {
							...
							if (this.type == 'popup') {
								uni.$emit('confirm', subVal, this.innerValue, this.onLoadData.vModel, this
									.selectData)
							} else {
								uni.$emit('confirm1', subVal, this.innerValue, this.onLoadData.vModel, this
									.selectData)
							}
							uni.navigateBack();
						}
