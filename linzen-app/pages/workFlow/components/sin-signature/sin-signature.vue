<template>
	<view class="signature-wrap">
		<view class="u-flex signImgBox">
			<image mode="scaleToFill" class="signImg" :src="prevView" @click="preViewImage(prevView)"></image>
			<view class="icon-zen icon-zen-signature signIcon" @tap="showSignature()" @touchstart="touchSignature()">
			</view>
			<view v-if="!prevView" class="txt">
				手写签名
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			preViewImage(img) {
				if (false) return uni.previewImage({
					urls: [img]
				})
			},
			onOK() {
