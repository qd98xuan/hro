<template>
  <div class="im-container" v-if="visible">
    <div class="header">
      ...
      <i class="icon-linzen icon-linzen-nav-close" @click="visible = false"></i>
    </div>
    <div class="main">
      <div class="chatBox">
        ...
        <div class="toolBox">
          <div class="toolBox-left">
            <a-popover placement="topLeft" trigger="click" overlayClassName="emoji-popover" v-model:visible="state.popoverVisible">
              <i class="link-text linzen-custom linzen-custom-emoticon-neutral" title="发送表情"></i>
              <template #content>
                <div class="emojiBox">
                  <ul class="emoji">
            ...
            <a-upload
              ...
              @change="handleChange">
              <i class="link-text linzen-custom linzen-custom-image" title="发送图片"></i>
            </a-upload>
          </div>
          <div class="toolBox-right" @click="openHistory">
            <i class="link-text icon-linzen icon-linzen-generator-time"></i>
            <span>聊天记录</span>
          </div>
        </div>
        <div class="writeBox">
          <linzen-textarea placeholder="点击这里，直接输入消息咨询" v-model:value.trim="messageContent" @keyup.enter.native="sendMessage()" />
          <div class="btns">
