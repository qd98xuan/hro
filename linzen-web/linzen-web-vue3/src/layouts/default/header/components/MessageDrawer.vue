<template>
  <BasicDrawer v-bind="$attrs" @register="registerDrawer" width="280px" class="full-drawer message-drawer" :closeFunc="handleClose">
    ...
    <ScrollContainer ref="scrollContainerRef" class="msg-list">
      <div v-for="(item, i) in list" :key="i" class="msg-list-item" :title="item.title" @click="readInfo(item)" v-if="list.length">
        <Badge :dot="item.isRead != '1'" status="warning">
          <i class="icon-linzen icon-linzen-xitong item-icon" v-if="item.type == 1"></i>
          <i class="icon-linzen icon-linzen-generator-notice item-icon notice-icon" v-else-if="item.type == 3"></i>
          <i class="icon-linzen icon-linzen-portal-schedule item-icon schedule-icon" v-else-if="item.type == 4"></i>
          <i class="icon-linzen icon-linzen icon-linzen-generator-flow item-icon flow-icon" v-else></i>
        </Badge>
...
<script lang="ts" setup>
  ...
  import { toDateValue } from '/@/utils/linzen';
  ...
  function bindScroll() {
    const bodyRef = scrollContainerRef.value;
