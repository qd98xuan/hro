  ...
  <Header :class="getHeaderClass">
    ...
    <div :class="`${prefixCls}-action`">
      ...
      <a-tooltip
        ...
        placement="bottom"
        :mouseEnterDelay="0.5"
        v-if="getUserInfo.systemIds && getUserInfo.systemIds.length > 1 && !getLinzenAppId()">
        <div :class="`${prefixCls}-action__item`" @click="openSystemTriggerDrawer(true, { list: getUserInfo.systemIds })">
          <i class="icon-linzen icon-linzen-systemToggle" />
        </div>
      </a-tooltip>
...
<script lang="ts">
  ...
  import { getLinzenAppId } from '/@/utils/linzen';
