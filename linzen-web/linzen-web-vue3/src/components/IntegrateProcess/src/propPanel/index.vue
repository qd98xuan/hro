  ...
  function transformFieldList(formFieldList) {
    ...
    const loop = (data, parent?) => {
      ...
      if (data.__vModel__ && data.__config__.projectKey !== 'table') {
        ...
        if (dyOptionsList.includes(config.projectKey) && config.dataType !== 'static') delete item.options;
        list.push(item);
      }
    ...
    outer: for (let i = 0; i < list.length; i++) {
      ...
      for (let j = 0; j < e.groups.length; j++) {
        ...
        if (
          ...
          (!['inputNumber', 'calculate'].includes(child.projectKey) || (['inputNumber', 'calculate'].includes(child.projectKey) && child.fieldValue !== 0))
        ) {
