    ...
    <ScrollContainer class="config-content">
      <a-form :colon="false" layout="vertical" :model="formConf" class="config-form">
        <a-form-item label="目标表单">
          ...
          <linzen-select v-model:value="formConf.flowId" :options="flowOptions" placeholder="请选择流程" showSearch class="!mt-10px" v-if="formConf.enableFlow" />
        </a-form-item>
        <a-form-item label="更新字段" class="!flex-nowrap">
          <div class="condition-main">
            ...
            <a-row :gutter="8" v-for="(item, index) in formConf.transferList" :key="index" class="mt-10px">
              ...
              <a-col :span="5">
                <linzen-select v-model:value="item.sourceType" :options="sourceTypeOptions" @change="item.sourceValue = ''" />
              </a-col>
              <a-col :span="8">
                <linzen-select
                ...
                <a-input v-model:value="item.sourceValue" placeholder="请输入值" allowClear v-if="item.sourceType === 2" />
              </a-col>
              <a-col :span="1" class="text-center" v-if="!item.required">
                <i class="icon-linzen icon-linzen-btn-clearn" @click="delTransferItem(index)" />
              </a-col>
            </a-row>
            <span class="link-text mt-10px inline-block" @click="addTransferItem()"><i class="icon-linzen icon-linzen-btn-add text-14px mr-4px"></i>添加字段</span>
          </div>
        </a-form-item>
        ...
        <a-form-item class="!flex-nowrap" v-if="integrateType != 1">
          ...
          <div class="condition-main">
            <div class="mb-10px" v-if="formConf.ruleList?.length">
              ...
              <linzen-select v-model:value="formConf.ruleMatchLogic" placeholder="请选择" :options="logicOptions" class="!w-68px" />
            </div>
            <div class="condition-list" v-for="(item, index) in formConf.ruleList" :key="index">
              <a-row :gutter="8">
                <a-col :span="8">
                  <linzen-select v-model:value="item.logic" placeholder="请选择" :options="logicOptions" />
                </a-col>
                <a-col :span="5">
              ...
              <a-row :gutter="8" v-for="(child, childIndex) in item.groups" :key="index + childIndex" class="mt-10px">
                ...
                <a-col :span="5">
                  <linzen-select class="w-full" v-model:value="child.symbol" placeholder="运算符号" :options="symbolOptions" disabled />
                </a-col>
                <a-col :span="10">
                  <linzen-select
        ...
        <a-form-item>
          ...
          <linzen-radio v-model:value="formConf.unFoundRule" :options="unFoundRuleOptions" />
        </a-form-item>
      </a-form>
    </ScrollContainer>
  </section>
...
<script lang="ts" setup>
  ...
  const emptyChildItem = { field: '', symbol: '==', fieldValueType: 1, fieldValue: undefined, projectKey: '', fieldValueLinzenKey: '', cellKey: +new Date() };
  const emptyItem = { logic: 'and', groups: [emptyChildItem] };

  const getRuleOptions = computed(() => props.formConf.formFieldList.filter(o => o.id.indexOf('-') < 0 && !notSupportList.includes(o.__config__.projectKey)));
