...
<script lang="ts" setup>
  ...
  defineOptions({ name: 'LinzenRelationFormAttr', inheritAttrs: false });
  ...
  onBeforeMount(() => {
    emitter.on('setRelationData', data => {
      if (!props.isStorage || !props.showField || !props.relationField || props.relationField != data.linzenRelationField) return;
