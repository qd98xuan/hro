...
<script lang="ts" setup>
  ...
  defineOptions({ name: 'LinzenRelationForm', inheritAttrs: false });
  ...
  const getColumns = computed<any[]>(() => {
    const columns = (props.columnOptions as any).map(o => ({ title: o.label, dataIndex: o.value }));
  ...
  function setValue(value) {
    ...
    if (!props.multiple) {
      getDataChange(props.modelId, value).then(res => {
        ...
        emitter.emit('setRelationData', { linzenRelationField: props.field, ...data, id: res.data.id });
      });
    }
  }
  function setNullValue() {
    ...
    generatorStore.setRelationData(relationData);
    emitter.emit('setRelationData', { linzenRelationField: props.field });
  }
  function onChange() {
    options.value = [];
    emit('update:value', '');
