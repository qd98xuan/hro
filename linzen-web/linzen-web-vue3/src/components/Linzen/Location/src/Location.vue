<template>
  <div :class="`${prefixCls}`">
    <a-button preIcon="icon-linzen icon-linzen-daka" @click="handleLocation()" v-if="!detailed">{{ innerValue.fullAddress ? '重新定位' : '添加定位' }}</a-button>
  ...
  <a-modal
    ...
    <div class="location-contain">
      ...
      <ScrollContainer ref="infiniteBody" class="around-contain" v-loading="loading && listQuery.currentPage == 1">
        ...
        <!-- 请选择允许微调范围内的地点 -->
        <linzen-empty class="!mt-100px" v-if="!list.length && !loading" />
      </ScrollContainer>
    </div>
  </a-modal>
</template>
...
<script lang="ts" setup>
  ...
  defineOptions({ name: 'LinzenLocation', inheritAttrs: false });
  const defaultValue = {
    lat: '',
    lng: '',
    name: '',
