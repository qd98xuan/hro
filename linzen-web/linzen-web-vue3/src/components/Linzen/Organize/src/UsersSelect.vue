<template>
  <div :class="[$attrs.class, 'select-tag-list']" v-if="buttonType === 'button'">
    <a-button preIcon="icon-linzen icon-linzen-btn-add" @click="openSelectModal">{{ modalTitle }}</a-button>
    <div class="tags">
      ...
      <div class="transfer-pane left-pane">
        ...
        <div class="transfer-pane__body transfer-pane__body-tab" v-if="selectType === 'all'">
          ...
          <template v-if="activeKey === '1'">
            ...
            <ScrollContainer v-loading="loading && pagination.currentPage === 1" v-else ref="infiniteBody">
              ...
              <linzen-empty v-if="!treeData.length" />
            </ScrollContainer>
          </template>
          <BasicTree
            class="tree-main"
        ...
        <div class="transfer-pane__body transfer-pane__body-tab" v-else>
          ...
          <ScrollContainer v-loading="loading && pagination.currentPage === 1" ref="infiniteBody">
            ...
            <linzen-empty v-if="!ableList.length" />
          </ScrollContainer>
        </div>
      </div>
      <div class="transfer-pane right-pane">
        ...
        <div class="transfer-pane__body">
          <ScrollContainer>
            ...
            <linzen-empty v-if="!selectedData.length" />
          </ScrollContainer>
        </div>
      </div>
    </div>
...
<script lang="ts" setup>
  ...
  const defaultSelectedList = [
    { id: 'department', type: 'department', fullName: '部门', icon: 'icon-linzen icon-linzen-tree-department1', children: [] },
    { id: 'position', type: 'position', fullName: '岗位', icon: 'icon-linzen icon-linzen-tree-position1', children: [] },
    { id: 'user', type: 'user', fullName: '用户', icon: 'icon-linzen icon-linzen-tree-user2', children: [] },
    { id: 'group', type: 'group', fullName: '分组', icon: 'icon-linzen icon-linzen-generator-group1', children: [] },
    { id: 'role', type: 'role', fullName: '角色', icon: 'icon-linzen icon-linzen-generator-role', children: [] },
    { id: 'system', type: 'system', fullName: '动态参数', icon: 'icon-linzen icon-linzen-tree-user2', children: [] },
  ];
  defineOptions({ name: 'LinzenUsersSelect', inheritAttrs: false });
