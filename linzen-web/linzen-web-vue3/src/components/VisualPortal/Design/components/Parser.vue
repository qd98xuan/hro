<template>
  <div class="h-full" @click.stop="handleClick(item)">
    <component :is="layouts" v-bind="getBindValue" v-if="item.projectKey !== 'card' && item.projectKey !== 'tab'" />
    <template v-if="item.projectKey === 'card'">
    ...
    <template v-if="item.projectKey === 'tab'">
      <a-tabs
...
<script lang="ts" setup>
  ...
  if (props.item.projectKey !== 'card') layouts.value = markRaw(defineAsyncComponent(() => getComponents(props.item.projectKey)));
  ...
  const getBindValue = computed(() => ({
    ...
    key: useNeedKeyList.includes(props.item.projectKey) ? props.item.renderKey : '',
  }));
