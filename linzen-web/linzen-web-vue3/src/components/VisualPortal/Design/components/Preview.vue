<template>
  <BasicModal
    ...
    :keyboard="false"
    destroyOnClose
    class="linzen-full-modal full-modal linzen-preview-portal-modal">
    <template #title>
  ...
  function filterList(layout) {
    const loop = list => {
      for (let i = 0; i < list.length; i++) {
        const item = list[i];
        if (!(Array.isArray(item.visibility) && item.visibility.includes('pc')) && item.projectKey) {
          list.splice(i, 1);
          i--;
        }
