      ...
      <div class="action-box">
        <div v-for="(item, listIndex) in componentsList" :key="listIndex" class="components-part">
          <a-dropdown placement="bottom">
            <div class="components-title">{{ item.title }}<i class="icon-linzen icon-linzen-arrow-down"></i></div>
            <template #overlay>
        ...
        <div v-if="showType == 'pc'">
          <grid-layout :layout.sync="layout" :row-height="40" v-if="layout.length">
            <grid-item
              ...
              <div class="mask" v-if="!noNeedMaskList.includes(item.projectKey)"></div>
                ...
                <draggable v-if="layout.length" class="drawing-board" :list="layout" :animation="340" group="componentsGroup" item-key="i">
                  <template #item="{ element }">
                    <div class="item-box m-10px portal-tabs-box" @click="handleClick(element)" :class="{ 'active-item': element.i === activeId }">
                      ...
                      <div class="mask" v-if="!noNeedMaskList.includes(element.projectKey)"></div>
...
<script lang="ts" setup>
  ...
  function handleAddComponent(item, type?, currentItem?) {
    ...
    let row = { ...clone, i, x, y };
    if (needDefaultList.includes(row.projectKey) && type != 'copy') row.option.defaultValue = getDefaultValue(row);
    if (currentItem) {
      if (currentItem.projectKey == 'card') currentItem.children = [row];
      if (currentItem.projectKey == 'tab') {
  ...
  function getDefaultValue(row) {
    ...
    if (projectKey == 'text') return 'LINZEN快速开发平台';
    if (projectKey == 'image') return 'https://app.cdn.linzensoft.com/image/3.2/banner1.png';
    if (projectKey == 'video') return 'https://cdn.linzensoft.com/2022/video/index_video.mp4';
    if (projectKey == 'barChart' || projectKey == 'lineChart' || projectKey == 'pieChart') return chartData.baseBarData;
    if (projectKey == 'radarChart') return chartData.radarData;
    if (projectKey == 'mapChart') return mapChartData;
    if (projectKey == 'rankList') return rankList;
    if (projectKey == 'timeAxis') return timeAxisList;
    if (projectKey == 'tableList') return tableList;
  }
    ...
    const loop = data => {
      if (data.children && item.children.length) {
        data.children.map(ele => {
          if (ele.projectKey) emitter.emit('eChart' + ele.i);
    ...
    return new Promise((resolve, reject) => {
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          ...
          if ((e.projectKey == 'video' || e.projectKey == 'image') && option.styleType == 2) {
            const val = e.projectKey == 'video' ? '视频' : '图片';
