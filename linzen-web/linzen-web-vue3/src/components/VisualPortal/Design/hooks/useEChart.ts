...
export function useEChart(activeData, chartRef) {
  ...
  function init() {
    if (activeData.dataType === 'dynamic') {
      ...
      getDataInterfaceRes(activeData.propsApi).then(res => {
        state.chartData = res.data;
        initChartData();
      });
    } else {
  ...
  function initChartData() {
    const projectKey = activeData.projectKey;
    ...
    typeArr.forEach((title, index) => {
      ...
      let obj = {
        name: title,
        type: type,
        stack: styleType == 5 || styleType == 6 || styleType == 7 || (styleType != 4 && projectKey == 'lineChart') ? title : 'total',
      };
      let chartArr = optionData.filter(item => title === item.type);
      if (projectKey == 'pieChart') {
      ...
      series: (() => {
        const list: any[] = (seriesData || []).map((ele: any, index) => {
          let label: any = {
            ...
            position: projectKey == 'pieChart' ? option.seriesLabelPosition : 'top',
          ...
          if (projectKey == 'barChart' || projectKey == 'lineChart' || projectKey == 'radarChart') itemStyle.color = getColor(index);
          if (projectKey == 'lineChart') {
    ...
    if (projectKey == 'barChart' || projectKey == 'lineChart') {
      if (projectKey == 'lineChart' && option.areaStyle) xAxis.boundaryGap = false;
