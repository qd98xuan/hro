    ...
    <a-table :data-source="state.list" :columns="state.columns" size="small" :pagination="false" rowKey="id" class="carousel-table">
      <template #bodyCell="{ column, record, index }">
        ...
        <template v-if="column.key === 'fullName'">
          <a-input v-model:value="record.fullName" placeholder="请输入名称" allowClear v-if="state.type === 1" />
        ...
        <template v-if="column.key === 'fixed'">
          <linzen-select v-model:value="record.fixed" :options="fixedOptions" placeholder="请选择" showSearch />
        </template>
        <template v-if="column.key === 'align'">
          <linzen-select v-model:value="record.align" :options="alignOptions" placeholder="请选择" showSearch />
        </template>
        <template v-if="column.key === 'width'">
          <a-input-number v-model:value="record.width" :min="0" :max="9999" />
        </template>
        <template v-if="column.key === 'fontSize'">
          <a-input-number v-model:value="record.fontSize" :min="0" :max="9999" />
        </template>
        ...
        <template v-if="column.key === 'fontColor'">
          <linzen-color-picker v-model:value="record.fontColor" size="small" />
        </template>
