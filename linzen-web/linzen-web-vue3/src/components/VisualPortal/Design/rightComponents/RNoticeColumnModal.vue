<template>
  <BasicModal v-bind="$attrs" width="1000px" class="linzen-modal-portal" @register="registerModal" title="选项设置" @ok="handleSubmit" destroyOnClose>
    <a-table :data-source="state.list" :columns="state.columns" size="small" :pagination="false" rowKey="id" class="carousel-table">
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'fixed'">
          <linzen-select v-model:value="record.fixed" :options="fixedOptions" placeholder="请选择" showSearch v-if="record.filedName !== 'classify'" />
          <span v-else></span>
        </template>
        <template v-if="column.key === 'align'">
          <linzen-select v-model:value="record.align" :options="alignOptions" placeholder="请选择" showSearch v-if="record.filedName !== 'classify'" />
          <span v-else></span>
        </template>
        <template v-if="column.key === 'width'">
          <a-input-number v-model:value="record.width" :min="0" :max="9999" v-if="record.filedName !== 'classify'" />
          <span v-else></span>
        </template>
        <template v-if="column.key === 'fontSize'">
          <a-input-number v-model:value="record.fontSize" :min="12" :max="25" v-if="record.filedName !== 'classify'" />
          <span v-else></span>
        </template>
        ...
        <template v-if="column.key === 'fontColor'">
          <linzen-color-picker v-model:value="record.fontColor" size="small" v-if="record.filedName !== 'classify'" />
          <span v-else></span>
        </template>
      </template>
    </a-table>
