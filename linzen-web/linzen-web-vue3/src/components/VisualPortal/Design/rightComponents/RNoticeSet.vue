<template>
  <a-collapse-panel>
    ...
    <template v-if="showType == 'pc'">
      <template v-if="activeData.option.styleType == 1">
        ...
        <a-form-item label="显示条数">
          <a-input-number v-model:value="activeData.option.noticeCount" :min="1" :max="50" @change="renderKeyChange" />
        </a-form-item>
        <a-form-item label="字体大小">
    ...
    <template v-else>
      ...
      <a-form-item label="显示条数">
        <a-input-number v-model:value="activeData.option.appCount" :min="1" :max="50" @change="renderKeyChange" />
      </a-form-item>
    </template>
