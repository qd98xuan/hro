      ...
      <template #bodyCell="{ column, record, index }">
        ...
        <template v-if="column.key === 'dataType'">
          <div class="flex items-center">
            ...
            <div class="flex-1">
              <LinzenUploadImgSingle v-if="record.dataType == 1" v-model:value="record.imageUrl" />
              <a-input v-else-if="record.dataType == 2" v-model:value="record.imageUrl" placeholder="请输入图片地址" allowClear />
              <interface-modal
                v-else
        ...
        <template v-if="column.key === 'textDefaultValue'">
          <a-input v-model:value="record[column.key]" placeholder="请输入文本内容" allowClear />
        </template>
          ...
          <div class="flex items-center">
            ...
            <div class="flex-1">
              ...
              <a-input v-if="record.linkType == 2" v-model:value="record.urlAddress" placeholder="请输入链接地址">
                <template #addonAfter>
                  <linzen-select v-model:value="record.linkTarget" :options="typeList" placeholder="请选择" class="!w-90px" />
                </template>
              </a-input>
            </div>
          </div>
