    ...
    <a-table :data-source="state.list" :columns="getColumns" size="small" :pagination="false" rowKey="id" :scroll="{ x: 1200, y: 478 }" class="carousel-table">
      ...
      <template #bodyCell="{ column, record, index }">
        ...
        <template v-if="column.key === 'num'">
          <a-input v-model:value="record.num" placeholder="请输入数值" :maxlength="10" allowClear />
        </template>
        ...
        <template v-if="column.key === 'icon'">
          <LinzenIconPicker v-model:value="record.icon" />
        </template>
        <template v-if="column.key === 'iconColor'">
          <linzen-color-picker v-model:value="record.iconColor" size="small" color-format="rgb" />
        </template>
