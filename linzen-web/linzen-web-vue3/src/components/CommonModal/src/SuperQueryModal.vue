<template>
  <BasicModal
    ...
    @ok="handleSubmit"
    destroyOnClose
    class="linzen-super-query-modal linzen-condition-modal">
          ...
          <template #content>
            <div class="plan-list" v-if="planList.length">
              <div class="plan-list-item" v-for="(item, i) in planList" :key="i" @click="selectPlan(item)">
                ...
                <i class="icon-linzen icon-linzen-nav-close" @click.stop="delPlan(item.id, i)"></i>
              </div>
            </div>
...
<script lang="ts" setup>
  ...
  const state = reactive<State>({
    planList: [],
    conditionList: [
      { logic: 'and', groups: [{ field: '', symbol: '', projectKey: '', fieldValueType: 2, fieldValue: undefined, fieldValueLinzenKey: '', cellKey: +new Date() }] },
    ],
