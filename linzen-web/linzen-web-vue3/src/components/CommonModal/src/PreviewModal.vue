<template>
  <BasicModal v-bind="$attrs" @register="registerModal" :title="title" :footer="null" :width="600" class="linzen-add-modal linzen-preview-modal">
    <div class="add-main">
      <div class="add-item add-item-left" @click="previewPc()">
        <i class="add-icon icon-linzen icon-linzen-pc"></i>
        <div class="add-txt">
          <p class="add-title">Web预览</p>
        </div>
      </div>
      <div class="add-item" @click="previewApp()">
        <i class="add-icon icon-linzen icon-linzen-mobile"></i>
        <div class="add-txt">
          <p class="add-title">App预览</p>
        </div>
      </div>
    </div>
    <BasicModal v-bind="$attrs" @register="registerQrModal" :title="title" :footer="null" :width="400" class="linzen-qrcode-modal">
      <linzen-qrcode :staticText="qrCodeText" :width="280" />
