        ...
        <div class="upload error-show">
          ...
          <div class="up_right">
            <p class="tip success-tip">
              正常数量条数：<span>{{ result.snum }}条</span>
            </p>
            <p class="tip error-tip">
              异常数量条数：<span>{{ result.fnum }}条</span>
            </p>
          </div>
        </div>
        <div class="conTips">
          <p>以下文件数据为导入异常数据</p>
          <a-button type="link" preIcon="icon-linzen icon-linzen-btn-download" @click="handleExportExceptionData()">导出异常数据</a-button>
        </div>
...
<script lang="ts" setup>
    ...
    if (state.activeStep == 0) {
      ...
      getImportPreview(state.apiUrl, { fileName: state.fileName })
        .then(res => {
          ...
          state.resultColumnsChildList = state.resultColumns.filter(o => o.projectKey === 'table' && o.children && o.children.length);
          const preHeaderList = cloneDeep(headerList);
          for (let i = 0; i < preHeaderList.length; i++) {
            if (preHeaderList[i].children?.length && preHeaderList[i]?.projectKey == 'table') {
          ...
          state.columnsChildList = state.columns.filter(o => o.projectKey === 'table' && o.children && o.children.length);
          state.btnLoading = false;
          state.activeStep += 1;
        })
        .catch(() => {
  ...
  function getMergeList(list) {
    list.forEach(item => {
      if (item.children && item.children.length && item?.projectKey == 'table') {
