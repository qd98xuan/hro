  ...
  <div class="common-container">
    <template v-if="config.popupType === 'dialog'">
      <a-modal
        ...
        <div class="linzen-common-search-box linzen-common-search-box-modal">
          ...
          <div class="linzen-common-search-box-right">
            <a-tooltip placement="top">
              ...
              <RedoOutlined class="linzen-common-search-box-right-icon" @click="initData" />
            </a-tooltip>
          </div>
        </div>
...
<script lang="ts" setup>
  ...
  const getColumns = computed<any[]>(() => {
    const columns = (props.config.columnOptions as any).map(o => ({ title: o.label, dataIndex: o.value, ellipsis: true, width: 100 }));
  ...
  function handleSubmit() {
    ...
    for (let i = 0; i < unref(selectRow).length; i++) {
      ...
      for (let j = 0; j < props.config.relationOptions.length; j++) {
        ...
        item[row.field] = row.type === 1 ? e[!unref(getIsDynamic) ? row.value + '_linzenId' : row.value] : row.value;
      }
      checked.push(item);
    }
    emit('select', checked);
  ...
  function getParamList() {
    ...
    for (let i = 0; i < templateJson.length; i++) {
      ...
      if (templateJson[i].projectKey === 'createUser') {
        templateJson[i].defaultValue = unref(getUserInfo).userId;
      }
