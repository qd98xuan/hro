    ...
    <a-form
      ...
      <template v-if="!['copy', 'remove'].includes(dataForm.value)">
        ...
        <a-form-item label="按钮图标">
          <linzen-icon-picker v-model:value="dataForm.btnIcon" placeholder="请选择" />
        </a-form-item>
      </template>
      ...
      <template v-if="!['copy', 'remove', 'add', 'batchRemove'].includes(dataForm.value)">
        <linzen-group-title content="动作设置" :bordered="false" />
        <a-form-item label="弹窗标题">
          <a-input v-model:value="actionConfig.popupTitle" placeholder="请输入" />
        </a-form-item>
        <a-form-item label="弹窗类型">
          <linzen-radio v-model:value="actionConfig.popupType" :options="popupTypeOptions" optionType="button" button-style="solid" class="right-radio" />
        </a-form-item>
        ...
        <template v-if="actionConfig.dataSource == 'dynamic'">
          <a-form-item label="远端数据">
            <interface-modal :value="actionConfig.interfaceId" :title="actionConfig.interfaceName" :hasPage="1" @change="onInterfaceChange" />
          </a-form-item>
          <a-form-item label="参数设置" style="margin-bottom: 0"></a-form-item>
          <a-table :data-source="actionConfig.templateJson" :columns="columns" size="small" :pagination="false">
        ...
        <a-form-item label="表单数据" v-else>
          <LinzenTreeSelect v-model:value="actionConfig.modelId" :options="treeData" placeholder="请选择" lastLevel allowClear @change="onModeIdChange" />
        </a-form-item>
        ...
        <a-table :data-source="actionConfig.columnOptions" :columns="getColumnOptionsColumns" size="small" :pagination="false">
          <template #bodyCell="{ column, record, index }">
            ...
            <template v-if="column.key === 'value'">
              ...
              <linzen-select v-else v-model:value="record.value" placeholder="请选择字段" :options="fieldOptions" showSearch @change="onChange($event, record)" />
            </template>
        ...
        <div class="table-add-action" @click="handleAddColumnOption()">
          <a-button type="link" preIcon="icon-linzen icon-linzen-btn-add">新增</a-button>
        </div>
        ...
        <a-table :data-source="actionConfig.relationOptions" :columns="relationOptionsColumns" size="small" :pagination="false">
          <template #bodyCell="{ column, record, index }">
            <template v-if="column.key === 'field'">
              <linzen-select v-model:value="record.field" placeholder="请选择表单字段" :options="childList" showSearch class="!w-124px" />
            </template>
            <template v-if="column.key === 'type'">
              <linzen-select v-model:value="record.type" placeholder="请选择映射来源" :options="typeOptions" @change="onTypeChange(record)" />
            </template>
            <template v-if="column.key === 'value'">
              <template v-if="record.type == 1">
                <a-auto-complete
                  ...
                  @focus="onFocus(record.value)"
                  @search="debounceOnSearch(record.value)" />
                <linzen-select v-else v-model:value="record.value" placeholder="请选择字段" :options="fieldOptions" showSearch />
              </template>
              <a-input v-model:value="record.value" placeholder="请输入值" allowClear v-else />
            </template>
            <template v-if="column.key === 'action'">
...
<script lang="ts" setup>
  ...
  const formFieldsOptions = computed(() => {
    const noAllowList = ['table', 'uploadImg', 'uploadFile', 'updateUser', 'updateTime'];
    let list: any[] = [];
    const loop = (data, parent?) => {
      ...
      if (data.__vModel__ && !noAllowList.includes(data.__config__.projectKey))
        list.push({
          id: data.__vModel__,
  ...
  function init(data) {
    ...
    childList.value = cloneDeep(data.children)
      .filter(o => o.__vModel__ && !noAllowSelectList.includes(o.__config__.projectKey))
      .map(o => ({
        id: o.__vModel__,
  ...
  function onInterfaceChange(id, row) {
    ...
    state.actionConfig.templateJson = row.templateJson
  ...
  function onRelationFieldChange(val, row) {
    if (!val) return (row.projectKey = '');
    let list = unref(formFieldsOptions).filter(o => o.__vModel__ === val);
    if (!list.length) return (row.projectKey = '');
    let item = list[0];
    row.projectKey = item.__config__.projectKey;
  }
