<template>
  <a-form-item label="合计设置">
    <linzen-switch v-model:value="activeData.showSummary" />
  </a-form-item>
  <a-form-item label="合计字段" v-if="activeData.showSummary">
    <linzen-select v-model:value="activeData.summaryField" placeholder="请选择" :options="childrenList" allowClear showSearch multiple />
  </a-form-item>
    ...
    <template #item="{ element }">
      <div class="custom-draggable-item">
        ...
        <div class="edit-btn custom-line-icon" @click="editBtnConfig(element)">
          <i class="icon-linzen icon-linzen-btn-edit" />
        </div>
      </div>
    </template>
  </draggable>
  ...
  <div class="custom-draggable-list">
    ...
    <div class="add-btn" v-if="activeData.footerBtnsList?.length < 5">
      <a-button type="link" preIcon="icon-linzen icon-linzen-btn-add" @click="addBtn()">添加按钮</a-button>
    </div>
  </div>
...
<script lang="ts" setup>
  ...
  import { inputComponents, selectComponents } from '/@/components/FormGenerator/src/helper/componentMap';
  ...
  const childrenList = computed(() => {
    const list = props.activeData.__config__.children.filter(
      o => ['input', 'inputNumber', 'calculate'].includes(o.__config__.projectKey) && o.__vModel__ && !o.useMask,
    );
    return list.map(o => ({ id: o.__vModel__, fullName: o.__config__.label }));
  });
  const getInputComponents = computed(() => inputComponents.filter(o => !noTableAllowList.includes(o.__config__.projectKey)));
  const getSelectComponents = computed(() => {
    const list = selectComponents.filter(o => !noTableAllowList.includes(o.__config__.projectKey));
