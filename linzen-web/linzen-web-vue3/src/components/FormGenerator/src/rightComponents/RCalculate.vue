<template>
  ...
  <a-form-item label="精度">
    <a-input-number v-model:value="activeData.precision" placeholder="精度" :min="0" :max="15" />
  </a-form-item>
...
<script lang="ts" setup>
  ...
  const calculateComps = computed<any[]>(() => {
    ...
    const loop = (data, parent?) => {
      ...
      if (data.__config__ && data.__config__.projectKey && data.__vModel__) {
        if (['inputNumber', 'calculate'].includes(data.__config__.projectKey)) {
