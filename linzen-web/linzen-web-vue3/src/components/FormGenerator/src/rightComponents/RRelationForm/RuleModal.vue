<template>
  <BasicModal v-bind="$attrs" @register="registerModal" title="填充规则配置" :width="600" class="linzen-transfer-modal" @ok="handleSubmit" destroyOnClose>
    <div class="condition-main">
      ...
      <span class="link-text my-10px inline-block" @click="addTransferItem()"><i class="icon-linzen icon-linzen-btn-add text-14px mr-4px"></i>添加字段</span>
    </div>
  </BasicModal>
</template>
<script lang="ts" setup>
  ...
  function init(data) {
    ...
    state.formFieldsOptions = cloneDeep(data.formFieldsOptions || []).filter(o => ['input', 'textarea'].includes(o.__config__.projectKey));
