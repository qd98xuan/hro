<template>
  ...
  <a-form-item label="弹窗类型" v-if="showType === 'pc'">
    <linzen-select v-model:value="activeData.popupType" placeholder="请选择" :options="popupTypeOptions" />
  </a-form-item>
  ...
  <a-form-item label="关联功能">
    <LinzenTreeSelect v-model:value="activeData.modelId" :options="treeData" placeholder="请选择" lastLevel allowClear @change="onModeIdChange" />
  </a-form-item>
  <a-form-item label="显示字段">
    <linzen-select v-model:value="activeData.relationField" placeholder="请选择" :options="fieldOptions" showSearch @dropdownVisibleChange="visibleChange" />
  </a-form-item>
  ...
  <a-form-item label="分页条数" v-if="activeData.hasPage">
    <linzen-radio v-model:value="activeData.pageSize" :options="pageSizeOptions" optionType="button" button-style="solid" class="right-radio" />
  </a-form-item>
  <a-divider></a-divider>
