<template>
  ...
  <template v-if="showType === 'pc'">
    <a-form-item label="选项样式">
      <linzen-radio v-model:value="activeData.optionType" :options="optionTypeOptions" optionType="button" buttonStyle="solid" />
    </a-form-item>
    <a-form-item v-if="activeData.optionType === 'button'" label="组件尺寸">
      <linzen-radio v-model:value="activeData.size" :options="sizeOptions" optionType="button" buttonStyle="solid" />
    </a-form-item>
  </template>
