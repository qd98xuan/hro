<template>
  ...
  <a-form-item label="前图标" v-show="showType === 'pc'">
    <linzen-icon-picker v-model:value="activeData.prefixIcon" placeholder="请选择" />
  </a-form-item>
  <a-form-item label="后图标" v-show="showType === 'pc'">
    <linzen-icon-picker v-model:value="activeData.suffixIcon" placeholder="请选择" />
  </a-form-item>
  ...
  <div v-for="(item, index) in activeData.__config__.regList" :key="index" class="reg-item">
    <span class="close-btn" @click="activeData.__config__.regList.splice(index, 1)">
      <i class="icon-linzen icon-linzen-nav-close" />
    </span>
