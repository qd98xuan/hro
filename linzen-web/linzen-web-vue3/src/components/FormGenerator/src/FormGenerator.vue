        ...
        <div class="components-list">
          <div v-for="(item, listIndex) in leftComponents" :key="listIndex" class="components-part">
            ...
            <draggable
              ...
              v-model="item.list"
              item-key="__config__.projectKey"
              filter=".disabled"
              :sort="false"
          ...
          <a-tooltip title="撤销">
            <div class="undo-btn">
              <a-button type="link" color="info" :disabled="!getCanUndo" @click="handleUndo(replaceDrawingList)">
                <i class="icon-linzen icon-linzen-report-icon-undo"></i>
              </a-button>
            </div>
          </a-tooltip>
          <a-tooltip title="重做">
            <div class="undo-btn">
              <a-button type="link" color="info" :disabled="!getCanRedo" @click="handleRedo(replaceDrawingList)">
                <i class="icon-linzen icon-linzen-report-icon-restore"></i>
              </a-button>
            </div>
          </a-tooltip>
        </div>
        <div class="action-bar-right">
          <a-button pre-icon="icon-linzen icon-linzen-pc" type="link" :class="{ 'unActive-btn': showType != 'pc' }" @click="showType = 'pc'">桌面端</a-button>
          <a-button pre-icon="icon-linzen icon-linzen-mobile" type="link" :class="{ 'unActive-btn': showType != 'app' }" @click="showType = 'app'">移动端</a-button>
          <a-button pre-icon="icon-linzen icon-linzen-video-play" type="link" class="unActive-btn" @click="handlePreview">预览</a-button>
          <a-button pre-icon="icon-linzen icon-linzen-clean" type="link" class="unActive-btn" @click="handleClear">清空</a-button>
        </div>
      </div>
...
<script lang="ts" setup>
  ...
  import { noVModelList, calculateItem, onlinePeculiarList, noTableAllowList } from './helper/config';
    ...
    return new Promise((resolve, reject) => {
      ...
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          ...
          if (config.layout === 'colFormItem' && !noVModelList.includes(config.projectKey) && !e.__vModel__) {
          ...
          if (config.projectKey === 'billRule') {
            if (!config.rule) {
              reject({ msg: '单据组件“单据模板”属性为必填项', target: 1 });
              break;
            }
          }
          if (config.projectKey === 'relationForm') {
            if (!e.modelId) {
          ...
          if (config.projectKey === 'popupSelect') {
            if (!e.interfaceId) {
          ...
          if (config.projectKey === 'popupTableSelect') {
            if (!e.interfaceId) {
          ...
          if (config.projectKey === 'autoComplete') {
            if (!e.interfaceId) {
          ...
          if (config.projectKey === 'uploadFile') {
            if (e.pathType === 'selfPath' && e.isAccount === 0 && !e.folder) {
              reject({ msg: '请设置自定义路径', target: 1 });
              break;
            }
          }
          if (config.projectKey === 'uploadImg') {
            if (e.pathType === 'selfPath' && e.isAccount === 0 && !e.folder) {
              reject({ msg: '请设置自定义路径', target: 1 });
              break;
            }
          }
          if (config.projectKey === 'link' || config.projectKey === 'iframe') {
  ...
  function createIdAndKey(item, parent: Nullable<GenItem> = null) {
    ...
    if (config.layout === 'colFormItem') {
      if (noVModelList.indexOf(config.projectKey) < 0 || item.isStorage) {
        item.__vModel__ = !generatorStore.getHasTable ? toggleVmodelCase(`${config.projectKey}Field${state.idGlobal}`) : '';
      }
      if (parent?.__vModel__ && parent?.__config__.projectKey === 'table') {
        item.__config__.parentVModel = parent.__vModel__;
      }
    } else if (config.layout === 'rowFormItem') {
      if (config.projectKey === 'table') {
        item.__vModel__ = toggleVmodelCase(`${config.projectKey}Field${state.idGlobal}`);
      }
      config.componentName = `${config.projectKey}${state.idGlobal}`;
  ...
  function shouldClone(_to, _from, target, _event, conf) {
    ...
    if (conf.__config__.projectKey === 'table') {
      if (noTableAllowList.includes(targetConf.__config__.projectKey)) {
        return false;
      }
    ...
    if (conf.__config__.projectKey === 'tableGridTd' && targetConf.__config__.projectKey === 'tableGrid') return false;
    return true;
  }
  // 预览表单
  ...
  function handleTableAddRow(element, insertPos, cloneRowIdx) {
    ...
    newRow.__config__.projectKey = 'tableGridTr';
  ...
  function initData() {
    ...
    if (props.formInfo.type == 1 || props.formInfo.type == 6) {
      ...
      state.leftComponents[0].list = [...state.leftComponents[0].list, ...onlinePeculiarList];
    }
