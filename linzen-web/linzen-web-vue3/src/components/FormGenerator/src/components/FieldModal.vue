      ...
      <template #bodyCell="{ column, record, index }">
        ...
        <template v-else>
          <template v-if="column.key === 'field'">
            <a-input v-model:value="record.field" placeholder="请输入列名" :maxlength="50" />
          </template>
          ...
          <template v-if="column.key === 'dataType'">
            <linzen-select v-model:value="record.dataType" placeholder="请选择" :options="options" />
          </template>
          <template v-if="column.key === 'allowNull'">
            <a-checkbox v-model:checked="record.allowNull" />
          </template>
          <template v-if="column.key === 'fieldName'">
            <a-input v-model:value="record.fieldName" placeholder="请输入说明" :maxlength="50" />
          </template>
        </template>
