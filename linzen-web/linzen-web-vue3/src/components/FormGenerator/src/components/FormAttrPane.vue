<template>
  <a-form :colon="false" labelAlign="left" :labelCol="{ style: { width: '100px' } }" class="right-board-form">
    <a-form-item label="表单尺寸">
      <linzen-radio v-model:value="formConf.size" :options="sizeOptions" optionType="button" button-style="solid" />
    </a-form-item>
    <a-form-item label="标签对齐">
      <linzen-radio v-model:value="formConf.labelPosition" :options="labelPositionOptions" optionType="button" button-style="solid" class="right-radio" />
    </a-form-item>
    <a-form-item label="标题宽度">
      <a-input-number v-model:value="formConf.labelWidth" placeholder="标题宽度" :min="0" />
    </a-form-item>
    <a-form-item label="标题后缀">
      <a-input v-model:value="formConf.labelSuffix" placeholder="标题后缀" />
    </a-form-item>
    <a-form-item label="栅格间隔">
      <a-input-number v-model:value="formConf.gutter" placeholder="栅格间隔" :min="0" />
    </a-form-item>
    <a-form-item label="弹窗类型" v-if="modelType != 3 && modelType != 6 && webType == 2 && !enableFlow">
      <linzen-select v-model:value="formConf.popupType" :options="popupTypeOptions" />
    </a-form-item>
    ...
    <a-form-item label="表单Class" v-if="modelType == 1 || modelType == 6">
      <linzen-select v-model:value="formConf.className" multiple allowClear :options="getClassNameOptions" showSearch />
    </a-form-item>
    <a-form-item label="主键策略">
      <linzen-select v-model:value="formConf.primaryKeyPolicy" :options="policyOptions" :disabled="!!formInfo.id" />
    </a-form-item>
    <a-form-item>
      <template #label>
        并发锁定
    ...
    <template v-if="modelType != 3 && modelType != 6 && !enableFlow">
      ...
      <div class="btn-config-list">
        <div class="btn-cell">
          ...
          <a-input v-model:value="formConf.confirmButtonText" />
        </div>
        <div class="btn-cell">
          <a-checkbox v-model:checked="formConf.hasCancelBtn" disabled>取消</a-checkbox>
          <a-input v-model:value="formConf.cancelButtonText" />
        </div>
        <div class="btn-cell" v-if="webType == 2 && !enableFlow && showType == 'pc'">
          ...
          <a-input v-model:value="formConf.printButtonText" />
        </div>
