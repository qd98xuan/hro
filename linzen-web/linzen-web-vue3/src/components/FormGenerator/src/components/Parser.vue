<script lang="tsx">
  ...
  import { getScriptFunc, onlineUtils, getDateTimeUnit, getTimeUnit } from '/@/utils/linzen';
  ...
  export default defineComponent({
    ...
    setup(props, { expose, emit }) {
        ...
        colFormItem(element) {
          ...
          const Item = (
            <render
              ...
              {...listeners}
              ref={config.projectKey === 'table' ? element.__vModel__ : undefined}
              relations={config.projectKey === 'table' ? state.relations : undefined}
            />
          );
          ...
          Object.keys(scheme.on).forEach(key => {
            ...
            listeners['on' + upperFirst(key)] = (...arg) => {
              if (key === 'change') {
                ...
                if (['popupSelect', 'relationForm'].includes(config.projectKey)) setTransferFormData(data, config);
      ...
      function getParamList(templateJson) {
            ...
            for (let i = 0; i < currRelations[key].length; i++) {
              ...
              if (vModel.includes('-')) {
                ...
                if (e.opType === 'setOptions') {
                  const query = { paramList: config.templateJson ? getParamList(config.templateJson) : [] };
      ...
      function comSet(field, prop, value) {
        ...
        const loop = list => {
          for (let i = 0; i < list.length; i++) {
            ...
            if (item.__vModel__ && item.__vModel__ === prop) {
              switch (field) {
                ...
                case 'endTime':
                  ...
                  break;
                case 'options':
                  if (dyOptionsList.indexOf(item.__config__.projectKey) > -1) item.options = value;
                  break;
                default:
            ...
            if (item.__config__ && item.__config__.projectKey !== 'table' && item.__config__.children && Array.isArray(item.__config__.children)) {
      ...
      function initFormData(componentList) {
        ...
        componentList.forEach(cur => {
          ...
          if (cur.__config__.projectKey == 'table') return;
      ...
      function buildRules(componentList) {
        componentList.forEach(cur => {
          ...
          if (config.children && config.projectKey !== 'table') buildRules(config.children);
        });
      }
      ...
      function buildOptions(componentList) {
        componentList.forEach(cur => {
          ...
          if (config.children && config.projectKey !== 'table') buildOptions(config.children);
        });
      }
      function buildRelations(componentList, relations) {
        componentList.forEach(cur => {
          ...
          if (config.projectKey === 'userSelect' && ['dep', 'pos', 'role', 'group'].includes(cur.selectType)) {
