...
export default defineComponent({
  ...
  setup(props, { emit, expose }) {
    ...
    function buildDataObject(confClone, dataObject, formData) {
      const projectKey = confClone.__config__.projectKey;
      ...
      if (['calculate', 'table', 'barcode', 'qrcode', 'popupSelect', 'popupTableSelect', 'autoComplete'].includes(projectKey)) {
        dataObject['formData'] = formData;
      }
      if (['table'].includes(projectKey)) {
        dataObject['relations'] = props.relations;
        dataObject['vModel'] = confClone.__vModel__;
        dataObject['ref'] = 'tableRef';
      }
      if (['relationForm', 'popupSelect'].includes(projectKey)) {
        dataObject['field'] = confClone.__config__.tableName
          ? confClone.__vModel__ + '_linzenTable_' + confClone.__config__.tableName + (confClone.__config__.isSubTable ? '0' : '1')
    ...
    return () => {
      ...
      const projectKey = upperFirst(props.conf.__config__.projectKey) === 'Table' ? 'InputTable' : upperFirst(props.conf.__config__.projectKey);
      const Comp = componentMap.get(projectKey as ComponentType) as ReturnType<typeof defineComponent>;
      if (!Comp) return null;
      const realDataObject = getRealProps(dataObject, props.conf.__config__.projectKey);
