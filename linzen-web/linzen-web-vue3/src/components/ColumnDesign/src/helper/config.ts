...
const getSearchType = item => {
  ...
  if (RangeList.includes(projectKey)) return 3;
  if (fuzzyList.includes(projectKey)) return 2;
  return 1;
};
const getSearchMultiple = projectKey => {
  const searchMultipleList = ['select', 'depSelect', 'roleSelect', 'userSelect', 'usersSelect', 'organizeSelect', 'posSelect', 'groupSelect'];
  return searchMultipleList.includes(projectKey);
};
...
const defaultColumnData = {
  ...
  customBtnsList: [],
  btnsList: [{ value: 'add', icon: 'icon-linzen icon-linzen-btn-add', label: '新增' }], // 按钮
  columnBtnsList: [
