    ...
    <ScrollContainer class="config-content" v-show="activeKey !== '3'">
      <a-form :colon="false" layout="vertical" :model="formConf" class="config-form" v-show="activeKey === '1'">
        ...
        <a-form-item label="审批设置">
          <linzen-radio v-model:value="formConf.assigneeType" :options="typeOptions" class="type-radio" @change="onAssigneeTypeChange" />
          ...
          <a-form-item label="审批节点" class="!mb-0" v-if="formConf.assigneeType === 5">
            <linzen-select v-model:value="formConf.nodeId" placeholder="请选择" showSearch :options="nodeOptions" />
          </a-form-item>
          ...
          <a-form-item class="!mb-0 !mt-10px" v-if="formConf.assigneeType === 6">
            ...
            <linzen-select v-model:value="formConf.extraRule" placeholder="请选择" :options="extraRuleOptions" />
          </a-form-item>
        </a-form-item>
        <a-form-item label="审批方式">
          <linzen-radio v-model:value="formConf.counterSign" :options="counterSignOptions" class="counterSign-radio" />
        </a-form-item>
        <a-form-item label="设置会签流转规则" v-if="formConf.counterSign == 1">
          <div class="countersign-cell mb-10px">
            ...
            <linzen-select v-model:value="formConf.counterSignConfig.auditType" :options="auditTypeOptions" class="!w-120px !mx-10px" />
          ...
          <div class="countersign-cell">
            ...
            <linzen-select v-model:value="formConf.counterSignConfig.rejectType" :options="rejectTypeOptions" class="!w-120px !mx-10px" />
        ...
        <a-form-item label="设置依次审批顺序" v-if="getCanSetApproversSort">
          <a-button preIcon="icon-linzen icon-linzen-btn-add" @click="openApproversSortListModal">设置审批顺序</a-button>
        </a-form-item>
        <a-form-item>
          ...
          <a-form-item label="设置退回到的节点" class="!mb-0px">
            <linzen-select v-model:value="formConf.rejectStep" placeholder="请选择" :options="rejectStepOptions" showSearch />
          </a-form-item>
        </a-form-item>
        ...
        <a-form-item label="抄送设置">
          ...
          <a-form-item class="!pt-10px">
            ...
            <linzen-select v-model:value="formConf.extraCopyRule" :options="extraCopyRuleOptions" />
          </a-form-item>
      ...
      <div v-show="activeKey === '2'">
        <a-form :colon="false" layout="vertical" :model="formConf" class="config-form">
          <a-form-item label="操作设置">
            ...
            <div class="btn-cell">
              ...
              <a-input v-model:value="formConf.cancelBtnText" />
            </div>
            <div class="btn-cell">
        ...
        <a-form :colon="false" :model="formConf" labelAlign="left" :labelCol="{ style: { width: '100px' } }" class="config-form">
          ...
          <a-form-item label="说明" class="normal-item-content">
            <linzen-textarea v-model:value="formConf.description" />
          </a-form-item>
        </a-form>
      </div>
      ...
      <div v-show="activeKey === '5'">
        ...
        <a-form :colon="false" :model="formConf" layout="vertical" class="config-form">
          <a-form-item>
            ...
            <linzen-select v-model:value="formConf.approveMsgConfig.on" :options="nodeNoticeOptions" />
          </a-form-item>
          ...
          <a-form-item>
            ...
            <linzen-select v-model:value="formConf.rejectMsgConfig.on" :options="nodeNoticeOptions" />
          </a-form-item>
          ...
          <a-form-item>
            ...
            <linzen-select v-model:value="formConf.copyMsgConfig.on" :options="nodeNoticeOptions" />
          </a-form-item>
          ...
          <a-form-item>
            ...
            <linzen-select v-model:value="formConf.overTimeMsgConfig.on" :options="nodeNoticeOptions" />
          </a-form-item>
          ...
          <a-form-item>
            ...
            <linzen-select v-model:value="formConf.noticeMsgConfig.on" :options="nodeNoticeOptions" />
          </a-form-item>
      ...
      <a-form :colon="false" :model="formConf" layout="vertical" class="config-form" v-show="activeKey === '6'">
        <a-form-item label="限时设置">
          <linzen-select v-model:value="formConf.timeLimitConfig.on" :options="nodeOverTimeMsgOptions" />
        </a-form-item>
        <a-form-item v-if="formConf.timeLimitConfig.on === 1">
          ...
          <linzen-select v-model:value="formConf.timeLimitConfig.nodeLimit" :options="overTimeOptions" />
          <div class="mt-12px" v-if="formConf.timeLimitConfig.nodeLimit === 2">
            ...
            <linzen-select v-model:value="formConf.timeLimitConfig.formField" :options="usedFormItems" showSearch allowClear placeholder="请选择" />
          </div>
        ...
        <a-form-item>
          ...
          <linzen-select v-model:value="formConf.overTimeConfig.on" :options="nodeOverTimeMsgOptions" />
        </a-form-item>
        ...
        <a-form-item>
          ...
          <linzen-select v-model:value="formConf.noticeConfig.on" :options="nodeOverTimeMsgOptions" />
        </a-form-item>
        ...
        <a-tab-pane :tab="item.title" v-for="(item, i) in state.assignList" :key="i">
          ...
          <a-row :gutter="10" v-for="(child, cIndex) in item.ruleList" :key="cIndex" class="mb-10px">
            ...
            <a-col :span="2" class="rule-cell">
              <a-button type="danger" @click="delTransmitRule(i, cIndex)"><i class="icon-linzen icon-linzen-nav-close" /></a-button>
            </a-col>
          </a-row>
          <div class="table-add-action" @click="addTransmitRule(i)">
            <a-button type="link" preIcon="icon-linzen icon-linzen-btn-add">新增规则</a-button>
          </div>
        </a-tab-pane>
      </a-tabs>
    </BasicModal>
...
<script lang="ts" setup>
  ...
  import { typeOptions, defaultStep, nodeNoticeOptions, nodeOverTimeMsgOptions, overTimeOptions, sourceTypeOptions } from '../helper/define';
  ...
  function getRealAssignList(assignList) {
    let newAssignList = props.prevNodeList.map(o => {
      ...
      formFieldList = formFieldList.filter(o => o.__config__.projectKey !== 'table');
      return {
        nodeId: o.nodeId,
