...
<script lang="ts" setup>
  ...
  // 所有表单字段
  const formFieldsOptions = computed(() => state.formFieldList.filter(o => o.__config__.projectKey !== 'table').map(o => ({ ...o, disabled: false })));
  // 不包含子表的表单字段
  ...
  function conditionNodeConfirm() {
    ...
    Object.assign(state.properties, state.conditionForm);
  ...
  const requiredDisabled = projectKey => {
    return ['billRule', 'createUser', 'createTime', 'updateTime', 'updateUser', 'currPosition', 'currOrganize', 'table'].includes(projectKey);
  };
  const getDataType = data => {
    if (!data.__config__ || !data.__config__.projectKey) return '';
    const projectKey = data.__config__.projectKey;
    if (['inputNumber', 'datePicker', 'rate', 'slider'].includes(projectKey)) {
      return 'number';
    } else if (['checkbox', 'uploadFile', 'uploadImg', 'cascader', 'organizeSelect', 'areaSelect'].includes(projectKey)) {
      return 'array';
    } else if (
    ...
    if (!formOperates.length || isUpdate) {
      for (let i = 0; i < state.formFieldList.length; i++) {
        ...
        res.push({
          ...
          requiredDisabled: requiredDisabled(data.__config__.projectKey) || data.__config__.required,
          projectKey: data.__config__.projectKey,
  ...
  function getConditionNodeFieldList() {
    ...
    if (!state.prevNodeList.length) {
      ...
      state.formFieldList = formFieldList.filter(o => o.__config__.projectKey !== 'table');
    }
  }
  // 获取上一节数据
