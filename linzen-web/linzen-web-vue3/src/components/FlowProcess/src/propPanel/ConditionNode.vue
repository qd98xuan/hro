    ...
    <ScrollContainer class="config-content condition-content">
      <div class="condition-main">
        <div class="mb-10px" v-if="formConf.conditions?.length">
          ...
          <linzen-select v-model:value="formConf.matchLogic" placeholder="请选择" :options="logicOptions" class="!w-68px" />
        </div>
        <div class="condition-list" v-for="(item, index) in formConf.conditions" :key="index">
          <a-row :gutter="8">
            ...
            <a-col :span="18">
              <a-button preIcon="icon-linzen icon-linzen-btn-add" @click="addItem(index)">添加条件</a-button>
              <a-button preIcon="icon-linzen icon-linzen-nav-close" class="ml-8px" @click="delGroup(index)">删除分组</a-button>
            </a-col>
          </a-row>
          <a-row :gutter="8" v-for="(child, childIndex) in item.groups" :key="index + childIndex" class="mt-10px">
            <a-col :span="9" class="!flex items-center">
              <linzen-select v-model:value="child.fieldType" :options="conditionTypeOptions" @change="onFieldTypeChange(child)" class="!w-85px" />
              <linzen-select
            ...
            <a-col :span="9" class="!flex items-center">
              <linzen-select v-model:value="child.fieldValueType" :options="conditionTypeOptions1" @change="onFieldValueTypeChange(child)" class="!w-85px" />
              <linzen-select
              ...
              <div class="flex-1 w-150px" v-if="child.fieldValueType === 2">
                ...
                <template v-else-if="child.projectKey === 'switch'">
                  <linzen-switch v-model:value="child.fieldValue" />
                </template>
                <template v-else-if="child.projectKey === 'timePicker'">
                  <linzen-time-picker v-model:value="child.fieldValue" :format="child.format || 'HH:mm:ss'" allowClear />
                </template>
                <template v-else-if="['datePicker', 'createTime', 'updateTime'].includes(child.projectKey)">
                  <linzen-date-picker v-model:value="child.fieldValue" :format="child.format" allowClear @change="onConditionDateChange($event, child)" />
                </template>
                <template v-else-if="['organizeSelect', 'currOrganize'].includes(child.projectKey)">
                  <linzen-organize-select v-model:value="child.fieldValue" allowClear @change="(val, data) => onConditionListChange(child, val, data)" />
                </template>
                <template v-else-if="['depSelect'].includes(child.projectKey)">
                  <linzen-dep-select v-model:value="child.fieldValue" allowClear @change="(val, data) => onConditionObjChange(child, val, data)" />
                </template>
                <template v-else-if="child.projectKey === 'roleSelect'">
                  <linzen-role-select v-model:value="child.fieldValue" allowClear @change="(val, data) => onConditionObjChange(child, val, data)" />
                </template>
                <template v-else-if="child.projectKey === 'groupSelect'">
                  <linzen-group-select v-model:value="child.fieldValue" allowClear @change="(val, data) => onConditionObjChange(child, val, data)" />
                </template>
                <template v-else-if="['posSelect', 'currPosition'].includes(child.projectKey)">
                  <linzen-pos-select v-model:value="child.fieldValue" allowClear @change="(val, data) => onConditionObjChange(child, val, data)" />
                </template>
                <template v-else-if="['userSelect', 'createUser', 'updateUser'].includes(child.projectKey)">
                  <linzen-user-select v-model:value="child.fieldValue" hasSys allowClear @change="(val, data) => onConditionObjChange(child, val, data)" />
                </template>
                <template v-else-if="['usersSelect'].includes(child.projectKey)">
                  <linzen-users-select v-model:value="child.fieldValue" allowClear @change="(val, data) => onConditionObjChange(child, val, data)" />
                </template>
        ...
        <span class="link-text mt-10px inline-block" @click="addGroup()"><i class="icon-linzen icon-linzen-btn-add text-14px mr-4px"></i>添加分组</span>
      </div>
      <a-form :colon="false" layout="vertical" :model="formConf" class="config-form !m-10px" v-if="formConf.conditionType === 1">
        <a-form-item label="转向节点">
          <linzen-select v-model:value="formConf.swerveNode" placeholder="请选择" :options="beforeNodeOptions" showSearch @change="onSwerveNodeChange" />
        </a-form-item>
      </a-form>
    </ScrollContainer>
...
<script lang="ts" setup>
  ...
  function onFieldValueChange(item, val, data) {
    ...
    item.fieldValueLinzenKey = val ? data.__config__.projectKey : '';
  }
