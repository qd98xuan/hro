    ...
    <ScrollContainer class="config-content" v-show="activeKey !== '3'">
      <a-form :colon="false" layout="vertical" :model="formConf" class="config-form" v-show="activeKey === '1'">
        ...
        <a-form-item v-if="flowType != 1">
          ...
          <linzen-users-select v-model:value="formConf.initiator" buttonType="button" modalTitle="添加发起人" multiple @labelChange="onLabelChange" />
        </a-form-item>
        <a-form-item label="抄送设置">
          <linzen-users-select v-model:value="formConf.circulateUser" buttonType="button" modalTitle="添加抄送人" multiple />
          <a-form-item class="!pt-10px">
            <template #label>抄送人范围<BasicHelp text="抄送人员增加人员选择范围附加条件" /></template>
            <linzen-select v-model:value="formConf.extraCopyRule" :options="extraRuleOptions" />
          </a-form-item>
        ...
        <a-form :colon="false" layout="vertical" :model="formConf" class="config-form">
          <a-form-item label="操作设置">
            <div class="btn-cell">
              ...
              <a-input v-model:value="formConf.submitBtnText" />
            </div>
            <div class="btn-cell">
            ...
            <div class="btn-cell">
              ...
              <a-input v-model:value="formConf.pressBtnText" />
            </div>
            <div class="btn-cell">
          ...
          <a-form-item label="标题设置">
            <linzen-radio v-model:value="formConf.titleType" :options="titleTypeOptions" />
          ...
          <a-form-item>
            ...
            <linzen-select v-model:value="formConf.errorRule" :options="errorRuleOptions" @change="formConf.errorRuleUser = []" />
            <linzen-user-select v-model:value="formConf.errorRuleUser" buttonType="button" multiple class="mt-10px" v-if="formConf.errorRule === 2" />
          </a-form-item>
          <a-form-item label="撤回后处理规则">
            <linzen-select v-model:value="formConf.revokeRule" :options="revokeRuleOptions" />
          </a-form-item>
          <a-form-item label="复活取值规则">
            <linzen-select v-model:value="formConf.resurgenceDataRule" :options="resurgenceDataRuleOptions" />
          </a-form-item>
        </a-form>
      ...
      <div v-show="activeKey === '4'">
        <a-form :colon="false" :model="formConf" labelAlign="left" :labelCol="{ style: { width: '70px' } }" class="config-form">
          ...
          <a-form-item label="">
            <linzen-select v-model:value="formConf.funcConfigRule" :options="funcConfigOptions" />
          </a-form-item>
          ...
          <a-form-item class="!-mt-12px" v-if="formConf.initFuncConfig.on">
            ...
            <a-table :data-source="formConf.initFuncConfig.templateJson" :columns="templateJsonColumns" size="small" :pagination="false">
                ...
                <template v-if="column.key === 'relationField'">
                  ...
                  <template v-else>
                    ...
                    <linzen-date-picker
                      ...
                      :disabled="record.sourceType == 3"
                      clearable />
                    <a-input v-else v-model:value="record.relationField" placeholder="请输入参数值" :disabled="record.sourceType == 3" clearable />
                  </template>
                </template>
              </template>
              <template #emptyText>
          ...
          <a-form-item class="!-mt-12px" v-if="formConf.endFuncConfig.on">
            ...
            <div class="ant-form-item-label mt-12px"><label class="ant-form-item-no-colon">参数设置</label></div>
            <a-table :data-source="formConf.endFuncConfig.templateJson" :columns="templateJsonColumns" size="small" :pagination="false">
              <template #bodyCell="{ column, record }">
                <template v-if="column.key === 'field'">
                  <span class="required-sign">{{ record.required ? '*' : '' }}</span>
                  {{ record.field }}{{ record.fieldName ? '(' + record.fieldName + ')' : '' }}
          ...
          <a-form-item class="!-mt-12px" v-if="formConf.flowRecallFuncConfig.on">
            ...
            <div class="ant-form-item-label mt-12px"><label class="ant-form-item-no-colon">参数设置</label></div>
            <a-table :data-source="formConf.flowRecallFuncConfig.templateJson" :columns="templateJsonColumns" size="small" :pagination="false">
              <template #bodyCell="{ column, record }">
                <template v-if="column.key === 'field'">
                  <span class="required-sign">{{ record.required ? '*' : '' }}</span>
                  {{ record.field }}{{ record.fieldName ? '(' + record.fieldName + ')' : '' }}
      ...
      <div v-show="activeKey === '5'">
        ...
        <a-form :colon="false" :model="formConf" layout="vertical" class="config-form">
          <a-form-item>
            ...
            <linzen-select v-model:value="formConf.waitMsgConfig.on" :options="noticeOptions" />
          </a-form-item>
          ...
          <a-form-item>
            ...
            <linzen-select v-model:value="formConf.endMsgConfig.on" :options="noticeOptions" />
          </a-form-item>
          ...
          <a-form-item>
            ...
            <linzen-select v-model:value="formConf.approveMsgConfig.on" :options="noticeOptions" />
          </a-form-item>
          ...
          <a-form-item>
            ...
            <linzen-select v-model:value="formConf.rejectMsgConfig.on" :options="noticeOptions" />
          </a-form-item>
          ...
          <a-form-item>
            ...
            <linzen-select v-model:value="formConf.copyMsgConfig.on" :options="noticeOptions" />
          </a-form-item>
          ...
          <a-form-item>
            ...
            <linzen-select v-model:value="formConf.overTimeMsgConfig.on" :options="noticeOptions" />
          </a-form-item>
          ...
          <a-form-item>
            ...
            <linzen-select v-model:value="formConf.noticeMsgConfig.on" :options="noticeOptions" />
          </a-form-item>
      ...
      <a-form :colon="false" :model="formConf" layout="vertical" class="config-form" v-show="activeKey === '6'">
        <a-form-item label="限时设置">
          <linzen-select v-model:value="formConf.timeLimitConfig.on" :options="overTimeMsgOptions" />
        </a-form-item>
        <a-form-item v-if="formConf.timeLimitConfig.on === 1">
          ...
          <linzen-select v-model:value="formConf.timeLimitConfig.nodeLimit" :options="overTimeOptions" disabled />
        ...
        <a-form-item>
          ...
          <linzen-select v-model:value="formConf.overTimeConfig.on" :options="overTimeMsgOptions" />
        </a-form-item>
        <a-form-item v-if="formConf.overTimeConfig.on === 1">
          ...
          <a-row class="leading-32px">
            <a-checkbox v-model:checked="formConf.overTimeConfig.overNotice">
              超时通知<BasicHelp text="勾选后才能进行超时消息发送（站内信系统默认发送，第三方超时消息需在流程通知内配置）" />
            </a-checkbox>
          </a-row>
          <a-row class="leading-32px">
            <a-checkbox v-model:checked="formConf.overTimeConfig.overAutoApprove">
              超时自动审批<BasicHelp text="当前审批节点表单必填字段为空会使工单流转失败，下一审批节点设置候选人员、选择分支时当前审批节点规则失效" />
            </a-checkbox>
          </a-row>
          <a-row>
          ...
          <a-row class="leading-32px">
            <a-checkbox v-model:checked="formConf.overTimeConfig.overEvent">超时事件<BasicHelp text="请在流程事件内配置超时事件" /></a-checkbox>
          </a-row>
          <a-row>
            <a-col :span="8" class="ant-form-item-label !pb-0"><label class="ant-form-item-no-colon !h-32px">超时次数(次)</label></a-col>
            <a-col :span="16">
        ...
        <a-form-item>
          ...
          <linzen-select v-model:value="formConf.noticeConfig.on" :options="overTimeMsgOptions" />
        </a-form-item>
        <a-form-item v-if="formConf.noticeConfig.on === 1">
          ...
          <a-row class="leading-32px">
            <a-checkbox v-model:checked="formConf.noticeConfig.overNotice">
              提醒通知<BasicHelp text="勾选后才能进行提醒消息发送（站内信系统默认发送，第三方超时消息需在流程通知内配置）" />
            </a-checkbox>
          </a-row>
          <a-row class="leading-32px">
            <a-checkbox v-model:checked="formConf.noticeConfig.overEvent">提醒事件<BasicHelp text="请在流程事件内配置提醒事件" /></a-checkbox>
          </a-row>
          <a-row>
...
<script lang="ts" setup>
  ...
  import { noticeOptions, overTimeMsgOptions, overTimeOptions, sourceTypeOptions } from '../helper/define';
  ...
  const titleContentOptions = computed(() => {
    ...
    const fields = props.usedFormItems.filter(
      o => o.__config__.projectKey === 'input' || o.__config__.projectKey === 'textarea' || o.__config__.projectKey === 'inputNumber',
    );
