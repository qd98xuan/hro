...
<script lang="ts" setup>
  ...
  import { getAmountChinese } from '/@/utils/linzen';
  import dayjs from 'dayjs';
  import { useBaseStore } from '/@/store/modules/base';
  import { getDateFormat } from '/@/utils/linzen';
  ...
  function handleReplaceValue(dom, tag) {
    ...
    for (let i = 0; i < dataList.length; i++) {
      ...
      if (elementText.includes('{') && elementText.includes('data-tag')) {
        ...
        if (tag == 'td') {
          let value = getValueFun(elementText);
          let spanText = elementText.match(/<span class="linzen-print-tag-span.*?[^}]}.*?<\/span>/);
  ...
  function replaceCommonValue() {
    const list: any[] = state.printTemplate.match(/<span class="linzen-print-tag-span.*?[^}]}.*?<\/span>/g) || [];
    for (let i = 0; i < list.length; i++) {
      if (list[i].includes('{') && list[i].includes('data-tag')) {
        let value = getValue(list[i]);
        let spanText = list[i].match(/<span class="linzen-print-tag-span.*?[^}]}.*?<\/span>/);
        if (spanText?.length) {
          spanText = spanText[0];
          let fieldStr = spanText.match(/{.*}/g);
  ...
  function getTableKey(dom) {
    ...
    for (let tdDom of tdDoms) {
      ...
      if (['thousands', 'isAmountChinese', 'linzenDictionary', 'linzenDate'].includes(dataTag)) {
  ...
  function replaceImage(dom, data) {
    ...
    for (let i = 0; i < list.length; i++) {
      const element = list[i];
      if (element.url) {
        let value = element.url.includes('http') ? element.url : apiUrl.value + element.url;
  ...
  function replaceSysValue() {
    ...
    if (recordList.length) {
      ...
      for (let i = 0; i < recordList.length; i++) {
        ...
        content += `<tr><td style="width: 20%;" data-mce-style="width: 20%;"><span class="linzen-print-tag-span linzen-tiny-color--common" contenteditable="false">${
          record.nodeName
        }</span></td><td style="width: 20%;" data-mce-style="width: 20%;"><span class="linzen-print-tag-span linzen-tiny-color--common" contenteditable="false">${
          record.userName
        }</span></td><td style="width: 20%;" data-mce-style="width: 20%;"><span class="linzen-print-tag-span linzen-tiny-color--common" contenteditable="false">${dayjs(
          record.handleTime,
        ).format(
          'YYYY-MM-DD HH:mm:ss',
        )}</span></td><td style="width: 20%;" data-mce-style="width: 20%;"><span class="linzen-print-tag-span linzen-tiny-color--common" contenteditable="false">${getHandleName(
          record.handleStatus,
        )}</span></td><td style="width: 20%;" data-mce-style="width: 20%;"><span class="linzen-print-tag-span linzen-tiny-color--common" contenteditable="false">${
