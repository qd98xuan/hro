  ...
  <div class="print-template-detail">
    <div class="print-template-detail__body">
      <div class="system-view-nav">
        <ScrollContainer>
          <div class="linzen-common-title"><h2>表单字段</h2></div>
          <BasicLeftTree
            ref="leftTreeRef"
            :showSearch="false"
      ...
      <PageSize @register="registerPageSize" :value="pageParam" @change="getEditConfig" />
...
<script lang="ts" setup>
  ...
  const state = reactive<State>({
    ...
    funData: [
      ...
      { fullName: '数据字典(字段,字典分类)', id: 'linzenDictionary' },
      { fullName: '日期选择(字段,日期格式)', id: 'linzenDate' },
    ],
  });
  ...
  function handleNodeClick(_item, node) {
    ...
    const tableParent = getCurrentParentByTag('table[data-linzen-table-tag="table"]');
    if (!tableParent) {
      if (item == 'linzenDictionary') return openDictionaryModal(true, { node });
  ...
  function reload(item, node) {
    unref(editor).insertContent(getSpanNode('linzenDictionary', node, item));
    state.content = unref(editor).getContent({ format: 'html' });
  }
  function getEditConfig(e) {
    ...
    state.initConfig = {
      ...
      content_style: `html {
        ...
        .linzen-tiny-color--customer {
          color: #005AF3;
        }
        .linzen-tiny-color--contacts {
          color: #1CB24C;
        }
        .linzen-tiny-color--business {
          color: #FF852F;
        }
        .linzen-tiny-color--contract {
          color: #FF4A43;
        }
        .linzen-tiny-color--receivables {
          color: #FFAE46;
        }
        .linzen-tiny-color--product {
          color: #00D0D4;
        }
        .linzen-tiny-color--common {
          color: #2362FB;
        }
      ...
      setup: editor => {
        editor.ui.registry.addButton('page', {
          text: `<i class="icon-linzen icon-linzen-orderExample" style="font-size:18px"></i>`,
          tooltip: '纸张大小',
          onAction: function () {
            openPageSizeModal(true);
          },
        });
        ...
        editor.on('keydown', e => {
          ...
          if (e.keyCode === 8 || e.keyCode == 46) {
            ...
            if (currentNode.getAttribute('data-linzen-table-td-tag') === 'value') {
              e.preventDefault();
              return false;
  ...
  function getSpanNode(id, node, item?) {
    const parent = node?.parent?.node?.id != null ? node.parent.node.id : 'null';
    ...
    if (id == 'linzenDictionary') return `<p style="display:inline-block" data-tag='linzenDictionary'>数据字典(${parent},"${item}")<p>`;
    if (id == 'linzenDate') return `<p style="display:inline-block" data-tag='linzenDate'>日期选择(${parent},"yyyy-MM-dd")<p>`;
    return `<span data-tag="${parent}.${id}" class="linzen-print-tag-span ${getSpanColorClass()}" contenteditable="false">{${id}}</span>`;
  }
  function getSpanColorClass() {
    ...
    return `linzen-tiny-color--${color}`;
  }
