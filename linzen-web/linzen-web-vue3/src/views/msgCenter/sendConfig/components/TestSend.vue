    ...
    <BasicTable @register="registerTable" :data-source="testSendData">
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'toUser'">
          <LinzenUserSelect v-model:value="record.toUser" multiple placeholder="请选择接收人" v-if="record.messageType != 'webhook'" />
        </template>
        ...
        <template v-if="column.key === 'parameterData'">
          <div class="variable-box" v-for="(item, index) in record.paramJson" :key="index">
            <a-input v-model:value="item.value" placeholder="请输入变量" allowClear />
          </div>
        </template>
      </template>
    </BasicTable>
