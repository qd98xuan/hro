      ...
      <a-row :gutter="20">
        ...
        <a-col :span="12">
          <a-form-item label="排序" name="sortCode">
            <a-input-number :min="0" :max="999999" v-model:value="dataForm.sortCode" />
          </a-form-item>
        </a-col>
        <a-col :span="24">
          ...
          <div class="parameter-box">
            <div class="left-pane">
              <div class="left-pane-list">
                  ...
                  <BasicTable @register="registerTable" :data-source="parameterList">
                    <template #bodyCell="{ column, record, index }">
                      ...
                      <template v-if="column.key === 'action' && !getIsSystem(record.field)">
                        <a-space :size="10">
                          <i class="icon-linzen icon-linzen-btn-edit" @click="addOrUpdateParameter(record)"></i>
                          <i class="icon-linzen icon-linzen-delete" @click="removeParameter(index)"></i>
                        </a-space>
                      </template>
                    </template>
                  </BasicTable>
                ...
                <div class="table-add-action" @click="addOrUpdateParameter()" v-if="dataForm.messageSource === '2'">
                  <a-button type="link" preIcon="icon-linzen icon-linzen-btn-add">{{ t('common.addText') }}</a-button>
                </div>
              </div>
            </div>
            <div class="right-pane" v-if="dataForm.messageType != 3 && dataForm.messageType != 7">
              ...
              <a-form-item name="content" v-if="dataForm.messageType == 2">
                ...
                <LinzenEditor v-model:value="dataForm.content" />
              </a-form-item>
              ...
              <a-row :gutter="20">
                ...
                <a-col :span="12" v-if="dataForm.messageType == 7">
                  <a-form-item label="跳转方式" name="wxSkip">
                    <linzen-select v-model:value="dataForm.wxSkip" placeholder="请选择" :options="wxSkipList" />
                  </a-form-item>
                </a-col>
              ...
              <div class="msg-pane">
                <div class="list">
                  <a-table :data-source="smsList" :columns="getSmsParameterColumns" size="small" :pagination="false">
                    ...
                    <template #bodyCell="{ column, record, index }">
                      <template v-if="column.key === 'smsField'">
                        <a-input v-model:value="record.smsField" :placeholder="dataForm.messageType == 3 ? '请输入短信变量' : '请输入变量'" allowClear />
                      </template>
                      <template v-if="column.key === 'field'">
                        <LinzenSelect
                      ...
                      <template v-if="column.key === 'action'">
                        <i class="icon-linzen icon-linzen-delete" danger @click="removeSmsData(index)"></i>
                      </template>
                    </template>
                  </a-table>
                </div>
                <div class="table-add-action" @click="addSmsData">
                  <a-button type="link" preIcon="icon-linzen icon-linzen-btn-add">添加一行</a-button>
                </div>
              </div>
            </div>
          </div>
...
<script lang="ts" setup>
  ...
  const state = reactive<State>({
    ...
    dataRule: {
      fullName: [{ required: true, message: '模板名称不能为空', trigger: 'blur' }],
      enCode: [
        { required: true, message: '模板编码不能为空', trigger: 'blur' },
        { validator: formValidate('enCode'), trigger: 'blur' },
      ],
      messageSource: [{ required: true, message: '请选择消息来源', trigger: 'change' }],
      ...
      wxSkip: [{ required: true, message: '跳转方式不能为空', trigger: 'blur' }],
      xcxAppId: [{ required: true, message: '关联小程序ID不能为空', trigger: 'blur' }],
    },
    parameterList: [],
    allParameterList: [],
    smsList: [],
