  ...
  <BasicModal v-bind="$attrs" @register="registerModal" title="流程评论" @ok="handleSubmit" destroyOnClose>
    <a-form :colon="false" :model="dataForm" :rules="dataRule" ref="formElRef">
      <a-form-item name="text">
        <linzen-textarea v-model:value="dataForm.text" placeholder="请输入评论" />
      </a-form-item>
      <a-form-item>
        <a-upload
          ...
          @change="handleChange">
          <a-button preIcon="icon-linzen icon-linzen-file-image">图片</a-button>
        </a-upload>
        <a-button preIcon="icon-linzen icon-linzen-btn-upload" class="ml-20px" @click="uploadFile">附件</a-button>
      </a-form-item>
        ...
        <div class="img-list">
          <div class="img-item" v-for="(item, i) in dataForm.image" :key="i">
            ...
            <div class="badge" @click.stop="handleImgRemove(i)">
              <i class="icon-linzen icon-linzen-nav-close"></i>
            </div>
          </div>
        </div>
      </a-form-item>
      <a-form-item>
        <linzen-upload-file ref="uploadFileRef" v-model:value="dataForm.file" type="workFlow" :limit="2" detailed class="comment-upload-file" />
      </a-form-item>
    </a-form>
  </BasicModal>
</template>
