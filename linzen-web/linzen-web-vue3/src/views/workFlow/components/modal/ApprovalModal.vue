    ...
    <a-form
      ...
      <template v-if="eventType === 'freeApprover'">
        <a-form-item label="加签人员" name="freeApproverUserId" :rules="[{ required: true, message: `必填`, trigger: 'change' }]">
          <linzen-user-select v-model:value="dataForm.freeApproverUserId" placeholder="请选择" allowClear />
        </a-form-item>
      ...
      <template v-if="eventType === 'audit' || (eventType === 'freeApprover' && dataForm.freeApproverType === 2)">
        <a-form-item
          ...
          <linzen-select v-model:value="dataForm.branchList" multiple placeholder="请选择" allowClear @change="onBranchChange" :options="branchList" showSearch />
        </a-form-item>
        <a-form-item
          ...
          <linzen-user-select v-model:value="item.value" multiple :placeholder="'请选择' + item.label" modalTitle="候选人员" v-else />
        </a-form-item>
      </template>
      ...
      <a-form-item label="抄送人员" name="copyIds" v-if="eventType !== 'freeApprover' && properties.isCustomCopy">
        <linzen-user-select v-model:value="dataForm.copyIds" multiple allowClear />
      </a-form-item>
      <template v-if="properties.hasOpinion">
        <a-form-item :label="`${label}意见`" name="handleOpinion">
          <linzen-textarea v-model:value="dataForm.handleOpinion" placeholder="请输入" />
