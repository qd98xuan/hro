      ...
      <a-row>
        ...
        <a-col :span="6" v-if="judgeShow('salesmanId')">
          <a-form-item label="业务人员" name="salesmanId">
            <linzen-user-select v-model:value="dataForm.salesmanId" placeholder="选择人员" @change="onSalesmanIdChange" :disabled="judgeWrite('salesmanId')" />
          </a-form-item>
        </a-col>
        <a-col :span="6" v-if="judgeShow('orderDate')">
          <a-form-item label="订单日期" name="orderDate">
            <linzen-date-picker v-model:value="dataForm.orderDate" format="YYYY-MM-DD HH:mm:ss" placeholder="选择日期" :disabled="judgeWrite('orderDate')" />
          </a-form-item>
        </a-col>
        ...
        <a-col :span="6" v-if="judgeShow('paymentMode')">
          <a-form-item label="付款方式" name="paymentMode">
            <linzen-select v-model:value="dataForm.paymentMode" :options="paymentOptions" placeholder="选择付款方式" :disabled="judgeWrite('paymentMode')" />
          </a-form-item>
        </a-col>
        ...
        <a-col :span="6" v-if="judgeShow('deliveryDate')">
          <a-form-item label="发货日期" name="deliveryDate">
            <linzen-date-picker v-model:value="dataForm.deliveryDate" placeholder="选择日期" :disabled="judgeWrite('deliveryDate')" />
          </a-form-item>
        </a-col>
        ...
        <a-col :span="24" v-if="judgeShow('fileJson')">
          <a-form-item label="相关附件" name="fileJson">
            <linzen-upload-file v-model:value="fileList" type="workFlow" :disabled="judgeWrite('fileJson')" />
          </a-form-item>
        </a-col>
        <a-col :span="24" v-if="judgeShow('description')">
          <a-form-item label="订单备注" name="description">
            <linzen-textarea v-model:value="dataForm.description" placeholder="订单备注" :disabled="judgeWrite('description')" />
          </a-form-item>
        </a-col>
      </a-row>
    </a-form>
    <a-tabs v-model:activeKey="activeKey" size="small" v-if="config.formOperates">
      <a-tab-pane key="1" tab="订单商品">
        ...
        <div class="table-add-action" @click="openGoodsModal(true, {})" v-if="!(config.disabled || judgeWrite('goodsList'))">
          <a-button type="link" preIcon="icon-linzen icon-linzen-btn-add">新增商品</a-button>
        </div>
      </a-tab-pane>
      <a-tab-pane key="2" tab="收款计划">
        ...
        <div class="table-add-action" @click="addPlan" v-if="!(config.disabled || judgeWrite('collectionPlanList'))">
          <a-button type="link" preIcon="icon-linzen icon-linzen-btn-add">新增计划</a-button>
        </div>
      </a-tab-pane>
    </a-tabs>
...
<script lang="ts" setup>
  ...
  import { toDecimal } from '/@/utils/linzen';
