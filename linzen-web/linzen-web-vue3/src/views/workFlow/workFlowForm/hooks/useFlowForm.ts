...
export function useFlowForm(data: UseFlowFormContext) {
  ...
  function updateDataRule() {
    if (!state.config.formOperates || !state.config.formOperates.length) return (selfState.dataRule = {});
    let newRules = {};
    for (let i = 0; i < state.config.formOperates.length; i++) {
      const item = state.config.formOperates[i];
      if (!item.id.includes('-')) {
        const newRulesItem = {
  ...
  function judgeRequired(id) {
    if (!state.config.formOperates || !state.config.formOperates.length) return false;
  ...
  function judgeShow(id) {
    if (state.config.opType == 4) return true;
    if (!state.config.formOperates || !state.config.formOperates.length) return true;
    let arr = state.config.formOperates.filter(o => o.id === id) || [];
  ...
  function judgeWrite(id) {
    if (state.config.disabled) return true;
    if (!state.config.formOperates || !state.config.formOperates.length) return false;
    let arr = state.config.formOperates.filter(o => o.id === id) || [];
    if (!arr.length) return true;
    let item = arr[0];
