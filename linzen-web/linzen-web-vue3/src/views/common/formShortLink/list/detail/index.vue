  ...
  function fillFormData(form, data) {
    ...
    const loop = (list, parent?) => {
      for (let i = 0; i < list.length; i++) {
        let item = list[i];
        if (item.__vModel__) {
          if (item.__config__.projectKey === 'relationForm' || item.__config__.projectKey === 'popupSelect') {
        ...
        if (['relationFormAttr', 'popupAttr'].includes(item.__config__.projectKey)) relationFormAttrList.push(item);
