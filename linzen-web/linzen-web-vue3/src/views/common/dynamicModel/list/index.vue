<template>
  <div class="linzen-content-wrapper">
    <div class="linzen-content-wrapper-left" v-if="columnData.type === 2">
    ...
    <div class="linzen-content-wrapper-center">
      <div class="linzen-content-wrapper-search-box" v-if="columnData.searchList?.length">
        <BasicForm
      ...
      <div class="linzen-content-wrapper-content">
        <BasicTable @register="registerTable" v-bind="getTableBindValue" ref="tableRef" @columns-change="handleColumnChange">
          <template #tableTitle>
            <a-button
              v-for="(item, i) in state.headerBtnsList"
              :key="item.value"
          ...
          <template #bodyCell="{ column, record, index }">
            <template v-if="column.flag === 'INDEX'">
              <div class="edit-row-action" v-if="columnData.type === 4 && !config.enableFlow">
                ...
                <i class="linzen-custom linzen-custom-arrow-expand" @click="handleRowForm(record)"></i>
              </div>
              ...
              <template v-if="record.rowEdit">
                ...
                <template v-else-if="['select', 'radio', 'checkbox'].includes(column.projectKey)">
                  <linzen-select
                    ...
                    :allowClear="column.clearable || ['radio', 'checkbox'].includes(column.projectKey)"
                ...
                <template v-else-if="column.projectKey === 'popupSelect' || column.projectKey === 'popupTableSelect'">
                  <linzen-popup-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple"
...
<script lang="ts" setup>
  ...
  import { getScriptFunc, onlineUtils, getDateTimeUnit, getTimeUnit, thousandsFormat } from '/@/utils/linzen';
  ...
  function handleLeftTreeSelect(id, _node, nodePath) {
    ...
    if (state.treeRelationObj?.projectKey && ['organizeSelect', 'cascader', 'areaSelect'].includes(state.treeRelationObj.projectKey)) {
  ...
  function addHandleForRowEdit() {
    let item = { rowEdit: true, id: 'linzenAdd' };
    ...
    const handlerInterface = data => {
      if (item.templateJson && item.templateJson.length) {
        item.templateJson.forEach(e => {
          ...
          e.defaultValue = e.sourceType == 1 && e.relationField == '@formId' ? record.id : e.sourceType == 1 ? value : e.relationField;
        });
      }
  ...
  function getExportList(list) {
    let exportList: any[] = [];
    for (let i = 0; i < list.length; i++) {
      if (list[i].projectKey === 'table') {
        if (state.columnData.type != 4) exportList.push(...list[i].children);
      } else if (list[i].projectKey === 'complexHeader') {
