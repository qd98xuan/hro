...
<script lang="ts" setup>
  ...
  import { getDateTimeUnit } from '/@/utils/linzen';
  ...
  function submitForm(data, callback) {
    ...
    if (state.config.customBtn) {
      if (state.config.templateJson?.length) {
        state.config.templateJson.forEach(e => {
          const value = data[e.relationField] || data[e.relationField] == 0 || data[e.relationField] == false ? data[e.relationField] : '';
          e.defaultValue = e.sourceType == 1 && e.relationField == '@formId' ? state.config.record.id : e.sourceType == 1 ? value : e.relationField;
        });
      }
      const query = { paramList: state.config.templateJson || [] };
