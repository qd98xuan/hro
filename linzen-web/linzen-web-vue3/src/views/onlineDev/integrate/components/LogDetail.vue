<template>
  <BasicModal v-bind="$attrs" @register="registerModal" title="日志详情" width="800px" :footer="null" class="linzen-log-detail-modal">
      ...
      <div class="left-board">
        <div class="item-box" :class="{ active: activeItem.id == item.id }" v-for="(item, index) in list" @click="handleClick(item, index)">
          <div class="top">
            ...
            <i class="icon icon-linzen icon-linzen-success" v-if="item.resultType" />
            <i class="icon icon-linzen icon-linzen-fail" v-else />
          </div>
          <div class="bottom">
            <i class="icon-linzen icon-linzen-clock pr-4px" v-if="item.type == 1" />
            <i class="icon-linzen icon-linzen-btn-refresh pr-4px" v-else />
