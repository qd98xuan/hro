<template>
  <BasicModal v-bind="$attrs" @register="registerModal" title="表单外链配置" :width="800" @ok="handleSubmit" class="linzen-shortLink-modal">
    <a-form :colon="false" :model="dataForm" :rules="rules" :labelCol="{ style: { width: '100px' } }" ref="formElRef" hideRequiredMark class="h-500px">
      <a-form-item label="外链填单" name="formUse">
        <a-space :size="15">
          <LinzenSwitch v-model:value="dataForm.formUse" />
            ...
            <template v-if="dataForm.alreadySave">
              ...
              <a-popover placement="bottomRight">
                <template #content>
                  ...
                  <a-button pre-icon="icon-linzen icon-linzen-download" block size="mini" @click="handleDownload(1)">下载</a-button>
                </template>
                <i class="linzen-custom linzen-custom-qrcode"></i>
              </a-popover>
            </template>
            <template v-else>
              <a-input-group compact>
        ...
        <a-row justify="start">
          <a-col>
            <a-form-item label="凭密码填写" name="formPassUse">
              <LinzenSwitch v-model:value="dataForm.formPassUse" />
            </a-form-item>
          </a-col>
          <a-col class="ml-15px">
      ...
      <template v-if="webType != '1'">
        ...
        <a-form-item label="公开查询" name="columnUse">
          <a-space :size="15">
            <LinzenSwitch v-model:value="dataForm.columnUse" />
              ...
              <template v-if="dataForm.alreadySave">
                ...
                <a-popover placement="bottomRight">
                  <template #content>
                    ...
                    <a-button pre-icon="icon-linzen icon-linzen-download" block size="mini" @click="handleDownload(2)">下载</a-button>
                  </template>
                  <i class="linzen-custom linzen-custom-qrcode"></i>
                </a-popover>
              </template>
          ...
          <a-row justify="start">
            <a-col>
              <a-form-item label="凭密码填写" name="columnPassUse">
                <LinzenSwitch v-model:value="dataForm.columnPassUse" />
              </a-form-item>
            </a-col>
            <a-col class="ml-15px">
...
<script lang="ts" setup>
  ...
  function getOptions(id) {
    getInfo(id).then(res => {
      ...
      list = list.filter(o => {
        if (!o.__config__ || !o.__config__.projectKey) return true;
        const projectKey = o.__config__.projectKey;
        if (validFieldsList.includes(projectKey) || (selectFieldsList.includes(projectKey) && o.__config__.dataType === 'static')) return true;
        return false;
      });
      const columnOptions = list.filter(o => !noColumnShowList.includes(o.__config__.projectKey) || o.isStorage);
      const searchOptions = list.filter(o => !noSearchList.includes(o.__config__.projectKey));
...
<style lang="less">
  .linzen-shortLink-modal {
    .shortLink-tip {
      color: @text-color-label;
    }

    .linzen-custom-qrcode {
      font-size: 32px;
      line-height: 29px;
      height: 29px;
