<template>
  <BasicPopup v-bind="$attrs" @register="registerPopup" title="项目任务" class="full-popup" :destroy-on-close="true">
    <BasicTable :columns="columns" @register="registerTable" class="linzen-task-table linzen-sub-table">
      <template #toolbar>
        <a-button type="primary" preIcon="icon-linzen icon-linzen-btn-add" @click="addOrUpdateHandle()">新建任务</a-button>
      </template>
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'signColor'">
          <i class="icon-linzen icon-linzen-extend-star" :style="'color:' + record.signColor + '!important;'" v-if="record.signColor"></i>
          <i class="icon-linzen icon-linzen-extend-star-o i-default" v-else></i>
        </template>
