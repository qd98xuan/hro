<template>
  <div class="linzen-content-wrapper bg-white">
    <div class="linzen-content-wrapper-center email-wrapper">
      <a-tabs v-model:activeKey="activeKey" tab-position="left" class="common-left-tabs">
        <template #leftExtra>
          <div class="tab-tem" @click="openFormPopup(true, { id: '' })"><i class="icon-linzen icon-linzen-btn-edit"></i>写邮件</div>
        </template>
        <a-tab-pane key="inBox">
          <template #tab><i class="icon-linzen icon-linzen-extend-envelope"></i>收件箱</template>
        </a-tab-pane>
        <a-tab-pane key="star">
          <template #tab><i class="icon-linzen icon-linzen-extend-star"></i>星标件</template>
        </a-tab-pane>
        <a-tab-pane key="draft">
          <template #tab><i class="icon-linzen icon-linzen-extend-exclamation-triangle"></i>草稿箱</template>
        </a-tab-pane>
        <a-tab-pane key="sent">
          <template #tab><i class="icon-linzen icon-linzen-extend-paper-plane"></i>已发送</template>
        </a-tab-pane>
        <template #rightExtra>
          <div class="tab-tem mt-2px" @click="openConfigModal(true, {})"><i class="icon-linzen icon-linzen-extend-cog"></i>邮箱配置</div>
        </template>
      </a-tabs>
      <div class="container">
        <div class="linzen-common-search-box">
          <BasicForm class="search-form" @register="registerSearchForm" @submit="handleSubmit" @reset="handleReset"></BasicForm>
          <div class="linzen-common-search-box-right">
            <a-button type="primary" preIcon="icon-linzen icon-linzen-btn-download" @click="receiveEmail" :loading="state.btnLoading">收邮件</a-button>
          </div>
        </div>
          ...
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'isRead'">
              <span v-if="activeKey === 'inBox' || activeKey === 'star'">
                <span v-if="record.isRead">
                  <i class="icon-linzen icon-linzen-extend-envelope-open-o i-default" title="点击标记为未读" @click="handleSetUnread(record)"></i>
                </span>
                <span v-else><i class="icon-linzen icon-linzen-extend-envelope text-warning" title="点击标记为已读" @click="handleSetRead(record)"></i></span>
              </span>
              <span v-else><i class="icon-linzen icon-linzen-extend-envelope-open-o i-default"></i></span>
            </template>
            <template v-if="column.key === 'attachment'">
              <span v-if="record.attachment && JSON.parse(record.attachment).length"><i class="icon-linzen icon-linzen-extend-paperclip i-default"></i></span>
              <span v-else></span>
            </template>
            ...
            <template v-if="column.key === 'starred'">
              <span v-if="record.starred">
                <i class="icon-linzen icon-linzen-extend-star text-warning img-star" title="点击取消星标" @click="handleSetUnStar(record)"></i>
              </span>
              <span v-else><i class="icon-linzen icon-linzen-extend-star-o i-default img-star" title="点击标记为星标邮件" @click="handleSetStar(record)"></i></span>
            </template>
...
<style lang="less" scoped>
  .email-wrapper {
    ...
    .linzen-common-search-box {
      margin-bottom: 10px;

      .linzen-common-search-box-right {
        right: 0;
        top: 10px;
      }
    }
