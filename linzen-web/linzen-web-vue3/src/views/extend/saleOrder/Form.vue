          ...
          <a-row>
            <a-col :span="8">
              <a-form-item label="发货仓库" name="goodsWarehouse">
                <linzen-select v-model:value="dataForm.goodsWarehouse" placeholder="发货仓库" :options="storeOptions" />
              </a-form-item>
            </a-col>
            <a-col :span="8">
            ...
            <a-col :span="8">
              <a-form-item label="收款方式" name="gatheringType">
                <linzen-select v-model:value="dataForm.gatheringType" placeholder="选择收款方式" :options="options" />
              </a-form-item>
            </a-col>
          </a-row>
          <a-row>
        ...
        <a-col :span="6">
          <a-form :labelCol="{ style: { width: '120px' } }">
            <a-form-item label="订货类型">
              <linzen-select v-model:value="orderType" placeholder="选择订货类型" :options="orderTypeOptions" />
            </a-form-item>
          </a-form>
        </a-col>
      </a-row>
    ...
    <BasicTable :data-source="dataForm.productEntryList" @register="registerTable">
      <template #bodyCell="{ column, record, index }">
        <template v-if="column.key === 'action'">
          <a-button type="link" preIcon="linzen-custom linzen-custom-plus" @click="handleAddItem(index)"></a-button>
          <a-button type="link" danger preIcon="linzen-custom linzen-custom-minus" @click="handleDelItem(record)"></a-button>
        </template>
