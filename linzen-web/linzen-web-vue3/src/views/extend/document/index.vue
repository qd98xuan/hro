<template>
  <div class="linzen-content-wrapper document-wrapper bg-white">
    <a-tabs v-model:activeKey="activeKey" tab-position="left" class="common-left-tabs">
      <a-tab-pane key="all">
        <template #tab><i class="icon-linzen icon-linzen-extend-folder-open"></i>全部文档</template>
        <div class="container">
          ...
          <div class="linzen-common-search-box">
            ...
            <div class="linzen-common-search-box-right">
              ...
              <a-button type="primary" preIcon="icon-linzen icon-linzen-btn-upload" @click="uploadFile()" class="ml-10px">上传文件</a-button>
            </div>
          </div>
            ...
            <template #bodyCell="{ column, record }">
              <template v-if="column.key === 'fullName'">
                <span v-if="record.type">
                  <i :class="'icon-linzen ' + toFileExt(record.fileExtension) + ' i-default'" />
                  {{ record.fullName }}
                </span>
                <span class="cursor-pointer" v-else @click="openFolder(record)">
                  <i class="icon-linzen icon-linzen-extend-folder text-warning" />
                  {{ record.fullName }}
                </span>
              </template>
              <template v-if="column.key === 'isShare'">
                <span v-if="record.type && record.isShare" title="共享文件"><i class="icon-linzen icon-linzen-extend-thumbs-up i-default"></i></span>
              </template>
      ...
      <a-tab-pane key="shareOut">
        <template #tab><i class="icon-linzen icon-linzen-extend-thumbs-up"></i>我的共享</template>
        <div class="container">
          <Breadcrumb class="!mb-10px">
      ...
      <a-tab-pane key="shareTome">
        <template #tab><i class="icon-linzen icon-linzen-extend-share"></i>共享给我</template>
        <div class="container">
          <Breadcrumb class="!mb-10px">
      ...
      <a-tab-pane key="trash">
        <template #tab><i class="icon-linzen icon-linzen-extend-trash"></i>回收站</template>
        <div class="container">
          <Breadcrumb class="!mb-10px">
...
<script lang="ts" setup>
  ...
  import { toFileSize } from '/@/utils/linzen';
  ...
  function toFileExt(ext) {
    ...
    if (ext == 'doc' || ext == 'docx') {
      return 'icon-linzen-file-word';
    }
    if (ext == 'xls' || ext == 'xlsx') {
      return 'icon-linzen-file-excel';
    }
    if (ext == 'ppt' || ext == 'pptx') {
      return 'icon-linzen-file-ppt';
    }
    if (ext == 'rar' || ext == 'zip') {
      return 'icon-linzen-file-zip';
    }
    if (ext == 'txt' || ext == 'log') {
      return 'icon-linzen-file-text';
    }
    if (ext == 'html' || ext == 'cs' || ext == 'xml') {
      return 'icon-linzen-file-code';
    }
    if (ext == 'gif' || ext == 'jpg' || ext == 'jpeg' || ext == 'bmp' || ext == 'png') {
      return 'icon-linzen-file-image1';
    }
    if (ext == 'pdf') {
      return 'icon-linzen-file-pdf';
    }
    if (ext == 'mp3') {
      return 'icon-linzen-file-audio';
    }
    if (ext == 'mp4') {
      return 'icon-linzen-file-movie';
    }
    return 'icon-linzen-file-blank';
  }
