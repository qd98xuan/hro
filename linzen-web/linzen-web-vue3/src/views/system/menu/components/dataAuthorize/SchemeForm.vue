<template>
  <BasicModal v-bind="$attrs" @register="registerModal" :title="getTitle" showOkBtn @ok="handleSubmit" class="linzen-condition-modal" destroy-on-close>
    <a-form :colon="false" :labelCol="{ style: { width: '0' } }" :model="dataForm" :rules="dataRule" ref="formElRef" class="condition-main">
      ...
      <div class="mb-10px" v-if="condition.length">
        ...
        <linzen-select v-model:value="dataForm.matchLogic" placeholder="请选择" :options="logicOptions" class="!w-68px" />
      </div>
      <a-form-item v-for="(item, index) in condition" :key="index" class="!mb-10px">
        <div>
          <a-row :gutter="8">
            ...
            <a-col :span="5">
              <a-button preIcon="icon-linzen icon-linzen-btn-add" class="w-full" @click="addItem(index)">添加条件</a-button>
            </a-col>
            <a-col :span="5">
          ...
          <a-row :gutter="8" v-for="(child, childIndex) in item.groups" :key="index + childIndex" class="mt-10px">
            <a-col :span="6">
              <linzen-select v-model:value="child.id" placeholder="选择字段" allowClear :options="fieldOptions" @change="onFieldChange($event, child)" />
            </a-col>
            <a-col :span="5">
              <linzen-select v-model:value="child.op" placeholder="选择符号" allowClear :options="child.opOptions" @change="onSymbolChange($event, child)" />
            </a-col>
            <a-col :span="12">
              <template v-if="child.conditionText === 'inputNumber'">
                <linzen-number-range v-model:value="child.value" :disabled="child.disabled" v-if="child.op == 'between'" />
                <linzen-input-number v-model:value="child.value" :disabled="child.disabled" placeholder="请输入" v-else />
              </template>
              ...
              <a-input v-model:value="child.value" placeholder="输入字段值" allowClear :readonly="child.readonly" :disabled="child.disabled" v-else />
            </a-col>
            <a-col :span="1">
      ...
      <a-form-item class="!mb-0">
        <span class="link-text inline-block" @click="addGroup()"><i class="icon-linzen icon-linzen-btn-add text-14px mr-4px"></i>添加分组</span>
      </a-form-item>
    </a-form>
  </BasicModal>
</template>
    ...
    getDataAuthorizeList({ moduleId: state.dataForm.moduleId })
      .then(res => {
        ...
        state.fieldOptions.map(o => {
          o.enCode = o.enCode.replace('_linzen_', '.');
          o.enCode = o.enCode.replace('linzen_', '');
        });
