<template>
  <BasicDrawer v-bind="$attrs" @register="registerDrawer" :title="title" :width="700" class="full-drawer">
    <a-tabs v-model:activeKey="activeKey" @change="onTabsChange" type="card" class="linzen-content-table-tabs" destroyInactiveTabPane>
        ...
        <BasicTable @register="registerSchemeTable">
          <template #tableTitle>
            ...
            <a-dropdown>
              ...
              <a-button type="link" :loading="loading" preIcon="icon-linzen icon-linzen-btn-add">常用方案<DownOutlined /></a-button>
            </a-dropdown>
          </template>
      ...
      <a-tab-pane :key="2" tab="字段管理">
        <BasicTable @register="registerFieldTable">
          <template #tableTitle>
            <a-button type="primary" preIcon="icon-linzen icon-linzen-btn-add" @click="addOrUpdateHandle()">{{ t('common.addText') }}</a-button>
            <a-button type="link" preIcon="icon-linzen icon-linzen-dbLink" v-if="type == '2'" @click="handleDataConnect">数据连接</a-button>
          </template>
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'action'">
              <TableAction :actions="getTableActions(record)" />
    ...
    return [
      {
        ...
        disabled: record.enCode === 'linzen_alldata' && state.activeKey == 1,
