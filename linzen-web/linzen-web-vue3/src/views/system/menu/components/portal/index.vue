<template>
  <BasicPopup v-bind="$attrs" @register="registerPopup" :title="title" class="full-popup" destroy-on-close>
    <div class="linzen-content-wrapper-search-box ml-10px mt-10px">
      <BasicForm class="search-form" @register="registerSearchForm" @submit="handleSubmit" @reset="handleReset" />
    </div>
    <a-tabs v-model:activeKey="listQuery.platform" type="card" class="linzen-content-table-tabs" destroyInactiveTabPane>
      <a-tab-pane key="Web" tab="Web门户">
        <BasicTable @register="registerWebTable" :columns="menuTableColumns" :searchInfo="getSearchInfo">
          <template #tableTitle>
            <a-button type="primary" preIcon="icon-linzen icon-linzen-btn-add" @click="addOrUpdateHandle()">添加</a-button>
          </template>
