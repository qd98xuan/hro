  ...
  <BasicModal v-bind="$attrs" @register="registerModal" :title="getTitle" showOkBtn @ok="handleSubmit" destroy-on-close>
    <BasicForm @register="registerForm">
      <template #icon="{ model, field }">
        <a-row type="flex">
          <div style="flex: 1; margin-right: 10px">
            <linzen-icon-picker v-model:value="model[field]" placeholder="请选择图标" />
          </div>
          <a-form-item-rest>
            <linzen-color-picker v-model:value="propertyJson.iconBackgroundColor" size="small" :predefine="predefineList" name="iconBackground" />
          </a-form-item-rest>
        </a-row>
      </template>
      <template #selfUrl>
        <a-form-item-rest>
          <div class="flex">
            <div class="flex-1 mr-10px">
              <a-input v-model:value="selfUrl" placeholder="访问地址" readonly />
            </div>
            <div>
      ...
      <template #navigationIcon="{ model, field }">
        <a-form-item-rest>
          <div class="flex">
            <LinzenUploadImgSingle v-model:value="model[field]" tipText="文字图标" subTipText="210 × 60 px" />
            <div class="ml-15px">
              <LinzenUploadImgSingle v-model:value="dataForm.workLogoIcon" tipText="Logo图标" subTipText="60 × 60 px" />
            </div>
          </div>
        </a-form-item-rest>
      </template>
...
<script lang="ts" setup>
  ...
  function getSelfUrl(enCode) {
    ...
    state.selfUrl = `${window.location.origin}/LINZEN_APP_${enCode}`;
  }
