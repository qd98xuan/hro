      ...
      <a-col span="14" offset="5">
        <BasicForm @register="registerForm">
          <template #taskCron="{ model, field }">
            <LinzenCron v-model:value="model[field]" @change="onCronChange" />
          </template>
          ...
          <template #interfaceId="{ model, field }">
            <interface-modal :value="model[field]" :title="interfaceName" @change="onInterfaceChange" />
          </template>
...
<script lang="ts" setup>
  ...
  function onInterfaceChange(val, row) {
    ...
    setFieldsValue({
      ...
      'executeContent.parameter': row.templateJson || '',
    });
