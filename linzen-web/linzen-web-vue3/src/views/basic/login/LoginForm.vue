<template>
    ...
    <Form class="enter-x" :model="formData" :rules="getFormRules" ref="formRef">
      ...
      <FormItem name="account" class="enter-x">
        <Input size="large" v-model:value="formData.account" :placeholder="t('sys.login.username')"
               class="fix-auto-fill" @blur="onAccountChange"/>
      </FormItem>
      <!-- 用户密码 -->
        ...
        <a-row type="flex" justify="space-between">
          ...
          <a-col class="sms-right">
            <a-tooltip :content="$t('sys.login.changeCode')" placement="bottom">
              <img class="codeImg" :alt="$t('sys.login.changeCode')" :src="apiUrl + codeImgUrl"
  ...
  <a-button type="primary" class="sso-login-btn enter-x" size="large" :loading="loading" @click="handleSsoLogin"
            v-show="isSso && !ssoLoading">登录
  </a-button>

  <BasicModal v-bind="$attrs" @register="registerSsoModal" title="登录" :footer="null" :width="1000"
              class="linzen-sso-modal" :closeFunc="onSsoModalClose">
    <iframe width="100%" height="100%" :src="ssoUrl" frameborder="0" v-if="showIframe"></iframe>
  </BasicModal>

  <BasicModal v-bind="$attrs" @register="registerTenantSocialModal" :closable="false" :footer="null" :width="700"
              class="linzen-tenant-social-modal">
    <div class="other-main">
      <div class="other-title">
        <div class="other-icon"><i class="icon-linzen icon-linzen-user"/></div>
...
const {formData, socialsList, isSso, ssoLoading, ssoUrl, showIframe, tenantSocialList, codeImgUrl} = toRefs(state);
  ...
  getTicket().then(res => {
    ...
    state.socialsList.forEach(item => {
      if (enname == item.enname) {
        const renderUrl = item.renderUrl.replace('LINZEN_TICKET', state.ssoTicket);
