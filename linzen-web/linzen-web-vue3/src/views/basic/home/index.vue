    ...
    <template v-if="!noData">
      <template v-if="!ajaxing">
        ...
        <div class="portal-layout-nodata" v-else>
          <linzen-empty :image="emptyImage" />
        </div>
      </template>
      <Setting @register="registerSettingDrawer" @refresh="refresh" />
      <a-button type="primary" preIcon="icon-linzen icon-linzen-left" class="setting-btn" size="large" @click="openSettingDrawer(true, { id: portalId })"></a-button>
    </template>
    <template v-else>
