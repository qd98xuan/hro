  ...
  <BasicDrawer v-bind="$attrs" @register="registerDrawer" title="字典分类管理" width="700px" class="full-drawer" destroy-on-close>
    <BasicTable @register="registerTable">
      <template #tableTitle>
        ...
        <linzen-upload-btn url="/api/system/DictionaryData/Actions/Import" accept=".bdd" @on-success="reload"></linzen-upload-btn>
      </template>
...
<script lang="ts" setup>
  ...
  function getTableActions(record): ActionItem[] {
    if (record.id === '0' || record.id === '1' || record.id === '2') return [];
    return [
      {
