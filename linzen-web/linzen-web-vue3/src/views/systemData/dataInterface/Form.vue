      ...
      <a-col :span="14" :offset="5">
        <BasicForm @register="registerForm">
          <template #hasPage="{ model, field }">
            <LinzenSwitch v-model:value="model[field]" :checkedValue="1" :unCheckedValue="0" @change="onHasPageChange" :disabled="hasPageDisabled" />
      ...
      <div class="middle-pane">
        <a-form :colon="false" :model="apiData" ref="formElRef">
          <a-form-item label="" name="url" :rules="pathRules" class="!mt-20px !mx-20px">
            <a-input v-model:value="apiData.url" placeholder="输入接口路径">
              <template #addonBefore>
                <div class="!w-80px">
                  <linzen-select v-model:value="apiData.method" :options="methodOptions" />
                </div>
              </template>
            </a-input>
          </a-form-item>
        ...
        <linzen-radio v-if="activeKey === 2" v-model:value="apiData.bodyType" class="!mb-20px !ml-20px" :options="bodyTypeList" @change="onBodyTypeChange" />
        <template v-if="getShowTableBox()">
          ...
          <div class="table-actions" @click="addHandle()">
            <a-button type="link" preIcon="icon-linzen icon-linzen-btn-add">{{ getAddTitle }}</a-button>
          </div>
        </template>
...
<script lang="ts" setup>
    ...
    getDataModelListAll(query)
      .then(res => {
        ...
        state.treeData = res.data.list.map(o => ({
          ...
          isLeaf: false,
          id: o.table,
          icon: o.type == 1 ? 'icon-linzen icon-linzen-view' : 'icon-linzen icon-linzen-generator-tableGrid',
        }));
      })
      .catch(() => {
        state.treeLoading = false;
  ...
  function handleNextFun() {
    state.activeStep += 1;
    // SQL操作
    if (state.dataForm.type === 1 && getIsLastStep()) setDataJsJson();
    // API操作
    if (state.dataForm.type === 3) {
