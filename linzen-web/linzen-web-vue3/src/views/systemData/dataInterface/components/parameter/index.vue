  ...
  <BasicTable @register="registerTable" :dataSource="list">
    <template #bodyCell="{ column, record, index }">
      <template v-if="column.key === 'field'">
        <a-input v-model:value="record.field" placeholder="请输入参数名" allowClear @change="onTableChange(record, index)" />
      </template>
      <template v-if="column.key === 'dataType'">
        <linzen-select v-model:value="record.dataType" :options="typeList" placeholder="请选择类型" @change="onTableChange(record, index)" />
      </template>
      ...
      <template v-if="column.key === 'defaultValue'">
        <linzen-select
          ...
          :placeholder="record.source === 1 ? '请选择参数值' : '请选择'"
          allowClear
