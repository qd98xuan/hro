    ...
    <a-table
      ...
      <template #bodyCell="{ column, record, index }">
        ...
        <template v-if="column.key === 'action'">
          <a-space :size="10">
            <i class="icon-linzen icon-linzen-btn-edit" @click="addOrUpdateHandle(record)"></i>
            <i class="icon-linzen icon-linzen-delete" @click="remove(index)"></i>
          </a-space>
        </template>
      </template>
    </a-table>
...
<script lang="ts" setup>
  ...
  import { getDataTypeText } from '/@/utils/linzen';
