  ...
  <div class="linzen-content-wrapper">
    <div class="linzen-content-wrapper-center">
      <div class="linzen-content-wrapper-content bg-white">
        <div class="p-20px">
          <LinzenAlert message="温馨提示：数据同步是由A数据库同步到B数据库。" type="warning" show-icon />
        </div>
        <a-form ref="fromRef" :model="dataForm" :rules="dataRule" :labelCol="{ style: { width: '140px' } }" class="">
          ...
          <a-form-item label="数据库连接 To" name="dbConnectionTo">
            <linzen-select v-model:value="dataForm.dbConnectionTo" show-search placeholder="请选择" :options="dbOptions" :fieldNames="{ options: 'children' }" />
            <a-button type="primary" @click="handleConnect" class="ml-10px">验证连接</a-button>
            <a-button preIcon="icon-linzen icon-linzen-btn-add" @click="handelAddConfigure" class="ml-10px">规则配置</a-button>
          </a-form-item>
        </a-form>
