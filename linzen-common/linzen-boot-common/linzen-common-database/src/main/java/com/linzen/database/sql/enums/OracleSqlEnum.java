...
@Getter
@AllArgsConstructor
public enum OracleSqlEnum implements SqlFrameBase {
...
//            + "WHERE a.TABLE_NAME = b.TABLE_NAME "
    ...
    ORDER_PAGE      ("SELECT * FROM " +
                    "(SELECT LINZEN_TEMP.*, ROWNUM LINZEN_RowNo " +
                        "FROM " +
                            "({selectSql}) LINZEN_TEMP " +
                        "ORDER BY {orderColumn} [DESC]) LINZEN_TAB " +
                    "WHERE LINZEN_TAB.LINZEN_RowNo BETWEEN {beginIndex} AND {endIndex}"),
