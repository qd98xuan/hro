...
@Getter
@AllArgsConstructor
public enum SQLServerSqlEnum implements SqlFrameBase {
...
//            " cast(COLUMNPROPERTY(a.id,a.name,'PRECISION') as varchar(50)) " + DbAliasConst.DATA_LENGTH + ", " +
    ...
    ORDER_PAGE     ("SELECT * " +
                            "FROM " +
                                "(SELECT TOP {endIndex} ROW_NUMBER() OVER(ORDER BY {orderColumn}) LINZEN_ROW,* " +
                            "FROM " +
                                "({selectSql}) AS LINZEN_TAB ORDER BY LINZEN_ROW) AS LINZEN_TEMP_TABLE " +
                            "WHERE " +
                                "LINZEN_ROW BETWEEN {beginIndex} AND {endIndex}"),
    // offset /fetch next关键字 2012版本及以上才有
    ORDER_PAGE2     (
            "SELECT \n" +
                    "\t*\n" +
                    "FROM\n" +
                    "\t({selectSql}) AS LINZEN_TAB \n" +
                    "ORDER BY\n" +
                    "\t{orderColumn} \n" +
