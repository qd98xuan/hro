...
public class ReportBuilder extends BasePagination implements ApplicationContextAware{
	 ...
	 * @param parameters
	 * @param connection
	 * @param searchForm		搜索表单信息
	 * @return
	 */
	public Report buildReports(ReportDefinition reportDefinition, Map<String,Object> parameters, Connection connection, SearchForm searchForm) {
		Report report = reportDefinition.newReport();
		long start=System.currentTimeMillis();
		Map<String,Dataset> datasetMap=buildDatasetss(reportDefinition, parameters, applicationContext,connection);
		Context context = new Context(this,report,datasetMap,applicationContext,parameters,hideRowColumnBuilder);
		long end=System.currentTimeMillis();
		String msg="~~~ Report select completed:"+(end-start)+"ms";
		...
		msg="~~~ Report compute completed:"+(end-start)+"ms";
		Utils.logToConsole(msg);
		return report;
	}

	public Report buildReports(ReportDefinition reportDefinition,Map<String,Object> parameters,Connection connection) {
		Report report = reportDefinition.newReport();
		long start=System.currentTimeMillis();
		Map<String,Dataset> datasetMap=buildDatasetss(reportDefinition, parameters, applicationContext,connection);
		Context context = new Context(this,report,datasetMap,applicationContext,parameters,hideRowColumnBuilder);
	...
	private Map<String,Dataset> buildDatasetss(ReportDefinition reportDefinition,Map<String,Object> parameters,ApplicationContext applicationContext,Connection connection){
...
//							conn = DriverManager.getConnection("jdbc:mysql://192.168.0.31:3306/linzen_dev?useUnicode=true&characterEncoding=utf-8&useSSL=false&allowMultiQueries=true&serverTimezone=UTC",
//									"linzen_dev", "CFPSNHyzttGzKTWi");
//						} catch (Exception e) {
//							e.printStackTrace();
//						}
//						throw new ReportComputeException("Buildin datasource ["+dsName+"] not exist.");
	...
	//修改buildReport方法，将数据库驱动传进去
	public Report buildReportToConnection(ReportDefinition reportDefinition,Map<String,Object> parameters,Connection connection) {
		Report report = reportDefinition.newReport();
		long start=System.currentTimeMillis();
		Map<String,Dataset> datasetMap=buildDatasetsToConnection(reportDefinition, parameters, applicationContext,connection);
		Context context = new Context(this,report,datasetMap,applicationContext,parameters,hideRowColumnBuilder);
		long end=System.currentTimeMillis();
		String msg="~~~ Report select completed:"+(end-start)+"ms";
		Utils.logToConsole(msg);
	...
	//修改buildDatasets方法，将数据库驱动传进去
	private Map<String,Dataset> buildDatasetsToConnection(ReportDefinition reportDefinition,Map<String,Object> parameters,ApplicationContext applicationContext,Connection connection){
		...
		for(DatasourceDefinition dsDef:datasources){
			if(dsDef instanceof JdbcDatasourceDefinition){
				System.out.println(((JdbcDatasourceDefinition) dsDef).getUrl());
				String dsName=dsDef.getName();
				Connection conn=null;
				try{
//					if(datasourceProviderMap.containsKey(dsName)){
//						conn=datasourceProviderMap.get(dsName).getConnection();
//					}
					String url = ((JdbcDatasourceDefinition) dsDef).getUrl();
					String driver = ((JdbcDatasourceDefinition) dsDef).getDriver();
					String username = ((JdbcDatasourceDefinition) dsDef).getUsername();
					String password = ((JdbcDatasourceDefinition) dsDef).getPassword();
...
//					}
					...
					BuildinDatasourceDefinition ds=(BuildinDatasourceDefinition)dsDef;
					List<Dataset> ls=ds.buildDatasets(connection, parameters);
					if(ls!=null){
						for(Dataset dataset:ls){
							datasetMap.put(dataset.getName(), dataset);
						}
