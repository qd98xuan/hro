		        		...
		        		if(obj!=null){
			        		if(obj instanceof String){
			        			...
			        			try{
			        				...
			        				anchor.setDx1(leftMargin * ImageUtils.EMU_PER_PIXEL);
			        				anchor.setDx2(width * ImageUtils.EMU_PER_PIXEL);
			        				anchor.setDy1(topMargin * ImageUtils.EMU_PER_PIXEL);
			        				anchor.setDy2(height * ImageUtils.EMU_PER_PIXEL);
			        			...
			        			if(base64Data!=null){
				    				...
				    				int leftMargin=0,topMargin=0;
									int widthColSpan = i + cellInfo.getColSpan() > columns.size() ? columns.size()-i : cellInfo.getColSpan();
									int wholeWidth=getWholeWidth(columns, i, widthColSpan);
									int heightRowSpan = rowIndex + cellInfo.getRowSpan() > rows.size() ? rows.size()-rowIndex : cellInfo.getRowSpan();
									int wholeHeight=getWholeHeight(rows, rowIndex, heightRowSpan);
				    				HorizontalAlignment align=style.getAlignmentEnum();
				    				if(align.equals(HorizontalAlignment.CENTER)){
				    					leftMargin=(wholeWidth-width)/2;
				    				}else if(align.equals(HorizontalAlignment.RIGHT)){
