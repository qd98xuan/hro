...
public class HtmlProducer{
	...
	public String produce(Context context,List<Page> pages,int columnMargin,boolean breakPage){
		...
		if(StringUtils.isNotBlank(bgImage)){
			bgStyle=";position: relative; z-index: 10;";
			sb.append("<div style='position: relative;width: "+tableWidth+"pt;'>");
			sb.append("<img src='"+bgImage+"' style='position: absolute;left: 0; right: 0;width:  100%;height: 100%;object-fit:cover; z-index: 1;' />");
		}
		if(breakPage){
	...
	private StringBuilder buildTable(Context context,List<Row> rows, List<Column> columns,Map<Row, Map<Column, Cell>> cellMap,boolean breakPage,boolean forPage) {
		String preview = ServletUtil.getRequest().getHeader("preview");
		boolean print = ObjectUtil.isNotEmpty(preview);
		StringBuilder sb=new StringBuilder();
		...
		if(breakPage){
			...
			sb.append("<table border='0' style='margin:auto;table-layout:fixed;border-collapse:collapse;width:"+tableWidth+"pt"+bgStyle+"'>");
		}
			...
			for(int j=0;j<colSize;j++){
				...
				if(StringUtils.isNotBlank(linkURL)){
					...
					if(linkURL.contains("${TOKEN}")){
						linkURL = linkURL.replaceAll("[$][{]TOKEN[}]", ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest().getHeader("Authorization"));
					}
				...
				if(obj instanceof Image){
					...
					String base64Data = "data:image/png;base64,"+chartData.retriveBase64Data();
					...
					if(print){
						sb.append("<image src=\"" + base64Data + "\" id=\"" + canvasId + "\" style=\"width:" + width + "px !important;height:" + height + "px !important\"></image>");
					}else {
